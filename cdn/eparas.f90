           ...
           DO m = -l,l
             ...
             ELSE
               ...
               DO natom = nt1,nt2
                 suma = suma + eigVecCoeffs%acof2(i,lm,0,natom,jsp)*CONJG(eigVecCoeffs%acof2(i,lm,0,natom,jsp))
                 sumb = sumb + eigVecCoeffs%acof2(i,lm,1,natom,jsp)*CONJG(eigVecCoeffs%acof2(i,lm,1,natom,jsp))
                 sumab= sumab + eigVecCoeffs%acof2(i,lm,0,natom,jsp) *CONJG(eigVecCoeffs%acof2(i,lm,1,natom,jsp))
                 sumba= sumba + eigVecCoeffs%acof2(i,lm,1,natom,jsp) *CONJG(eigVecCoeffs%acof2(i,lm,0,natom,jsp))
               ENDDO
        ...
        if (banddos%map_atomtype(n)==0) then
          DO i = (skip_t+1),noccbd
            ...
            DO natom = sum(atoms%neq(:n-1))+1,sum(atoms%neq(:n))
              suma=suma+dot_product(eigVecCoeffs%acof2(i,l* (l+1)-l:l* (l+1)+l,0,natom,jsp),eigVecCoeffs%acof2(i,l* (l+1)-l:l* (l+1)+l,0,natom,jsp))
              sumb=sumb+dot_product(eigVecCoeffs%acof2(i,l* (l+1)-l:l* (l+1)+l,1,natom,jsp),eigVecCoeffs%acof2(i,l* (l+1)-l:l* (l+1)+l,1,natom,jsp))
            ENDDO
