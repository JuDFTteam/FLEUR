...
CONTAINS
   SUBROUTINE vacden(vacuum,stars,input,cell,atoms,noco,nococonv,banddos,&
      ...
      ! STM/unused arguments
      !TYPE(t_gVacMap),  INTENT(IN)    :: gVacMap ! STM
      !REAL,    INTENT (IN)    :: eig(input%neig)
      !TYPE(t_sym),      INTENT(IN)    :: sym

      COMPLEX :: aa, ab, av, ba, bb, bv, t1, factorx, factory, c_1, tempCmplx
      REAL    :: arg, const, eps, ev, evacp, phs, phsp, qout, scale, sign, &
      ...
      !REAL                 :: ddui,dduj,dduei,dduej,ui_1,uei_1,uj_1,uej_1,wronk_1
      !COMPLEX              :: aa_1,ab_1,ba_1,bb_1,av_1,bv_1,aae,bbe,abe,bae,aaee,bbee,abee,baee,d
      !INTEGER              :: mapg2k(lapw%dim_nv2d())
      !INTEGER, PARAMETER   :: n2max=13
      !REAL,    PARAMETER   :: emax=2.0/hartree_to_ev_const
      !REAL,    ALLOCATABLE :: du(:),ddu(:,:),due(:),ddue(:,:),dummy(:)
      ...
      IF ( noco%l_noco .AND. (.NOT. noco%l_ss) ) THEN
         ...
         END DO
         DO k = 1,lapw%nv(1)
            !lapw%k3(k,2) = lapw%k3(k,1) ! I just removed this and made lapw INTENT(IN).
            map2(k,2) = map2(k,1)
         END DO
      END IF
