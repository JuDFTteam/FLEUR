            ...
            DO l = 0, lmax
               IF(l == 0) THEN
                  ...
                  DO spin = 1, input%jspins
                     c(0) = c(0) + eigVecCoeffs%abcof(iBand,0,0,iAtom,spin)*CONJG(eigVecCoeffs%abcof(iBand,0,0,iAtom,spin)) &
                                 + eigVecCoeffs%abcof(iBand,0,1,iAtom,spin)*CONJG(eigVecCoeffs%abcof(iBand,0,1,iAtom,spin)) &
                                    *usdus%ddn(0,iType,spin)

                     DO ilo  = 1, atoms%nlo(iType)
                        IF(atoms%llo(ilo,iType) /= 0) CYCLE
                        c(0) = c(0) + 2*REAL(eigVecCoeffs%abcof(iBand,0,0,iAtom,spin)*eigVecCoeffs%ccof(0,iBand,ilo,iAtom,spin))* usdus%uulon(ilo,iType,spin) &
                                    + 2*REAL(eigVecCoeffs%abcof(iBand,0,1,iAtom,spin)*eigVecCoeffs%ccof(0,iBand,ilo,iAtom,spin))* usdus%dulon(ilo,iType,spin)
                        DO ilop  = 1, atoms%nlo(iType)
                           IF(atoms%llo(ilo,iType) /= 0) CYCLE
                           c(0) = c(0) + eigVecCoeffs%ccof(0,iBand,ilo,iAtom,spin)*CONJG(eigVecCoeffs%ccof(0,iBand,ilop,iAtom,spin))*usdus%uloulopn(ilo,ilop,iType,spin)
                        ENDDO
                     ENDDO
                  ENDDO
               ELSE
                  DO jj = 1, 2
                     ...
                     DO WHILE(mj <= j)
                        ...
                        IF(ABS(mup) <= l) THEN
                           ...
                           aup   = facup   * eigVecCoeffs%abcof(iBand,lmup,0,iAtom,1)
                           bup   = facup   * eigVecCoeffs%abcof(iBand,lmup,1,iAtom,1)
                        ELSE
                           aup = 0.0
                           bup = 0.0
                        ENDIF
                        ...
                        IF(ABS(mdown) <= l) THEN
                           ...
                           adown = - facdown * eigVecCoeffs%abcof(iBand,lmdown,0,iAtom,spin)
                           bdown = - facdown * eigVecCoeffs%abcof(iBand,lmdown,1,iAtom,spin)
                        ELSE
                           adown = 0.0
                           bdown = 0.0
                        ENDIF
                        ...
                        DO ilo = 1, atoms%nlo(iType)
                           ...
                           IF(ABS(mup) <= l) THEN
                              cup = facup  * eigVecCoeffs%ccof(INT(mup),iBand,ilo,iAtom,1)
                           ELSE
                              cup = 0.0
                           ENDIF

                           IF(ABS(mdown) <= l) THEN
                              cdown = - facdown  * eigVecCoeffs%ccof(INT(mdown),iBand,ilo,iAtom,spin)
                           ELSE
                              cdown = 0.0
                           ENDIF
                           ...
                           DO ilop = 1, atoms%nlo(iType)
                              ...
                              IF(ABS(mup) <= l) THEN
                                 cupp = facup  * eigVecCoeffs%ccof(INT(mup),iBand,ilop,iAtom,1)
                              ELSE
                                 cupp = 0.0
                              ENDIF

                              IF(ABS(mdown) <= l) THEN
                                 cdownp = - facdown  * eigVecCoeffs%ccof(INT(mdown),iBand,ilop,iAtom,spin)
                              ELSE
                                 cdownp = 0.0
                              ENDIF
