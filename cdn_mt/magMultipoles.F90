...
CONTAINS
  SUBROUTINE magMultipoles(sym,stars, atoms,cell, sphhar, vacuum, input, noco,nococonv,outden)
    ...
    CALL cden%init(stars%ng3,atoms%jmtd,sphhar%nlhd,atoms%ntype,0,input%jspins,.FALSE.,.FALSE.,1001,&
         vacuum%nmzd,vacuum%nmzxyd,stars%ng2)
    DO n=1,3
       CALL m_den(n)%init(stars%ng3,atoms%jmtd,sphhar%nlhd,atoms%ntype,0,input%jspins,.FALSE.,.FALSE.,1001,&
            vacuum%nmzd,vacuum%nmzxyd,stars%ng2)
    ENDDO
    CALL div%init(stars%ng3,atoms%jmtd,sphhar%nlhd,atoms%ntype,0,input%jspins,.FALSE.,.FALSE.,1001,&
    vacuum%nmzd,vacuum%nmzxyd,stars%ng2)
