...
foreach(ADD_String "-lhdf5_fortran;-lhdf5"
   ...
   if (NOT FLEUR_USE_HDF5)
     ...
     try_compile(FLEUR_USE_HDF5 ${CMAKE_BINARY_DIR} ${CMAKE_SOURCE_DIR}/cmake/tests/test_HDF5.f90
	  ...
	  if ("$ENV{VERBOSE}")
        message("HDF5 compile test: ${FLEUR_USE_HDF5}\nLINK_LIBRARIES ${TEST_LIBRARIES}\n${compile_output}")
     endif()
...
if (DEFINED CLI_FLEUR_USE_HDF5)
   if (CLI_FLEUR_USE_HDF5)
	   ...
	   try_compile(MPI_C_FOUND ${CMAKE_BINARY_DIR} ${CMAKE_SOURCE_DIR}/cmake/tests/test_MPI.c)
           try_compile(MPI_CXX_FOUND ${CMAKE_BINARY_DIR} ${CMAKE_SOURCE_DIR}/cmake/tests/test_MPI.cpp)
	   message("Test for HDF5 compilation with MPI:")
	   message("FLEUR_USE_MPI: ${FLEUR_USE_MPI}")
	   message("MPI for CXX found: ${MPI_CXX_FOUND}")
           message("MPI for C found: ${MPI_C_FOUND}")
	   if (FLEUR_USE_MPI AND MPI_C_FOUND AND MPI_CXX_FOUND)
	      set(HDF5_ENABLE_PARALLEL ON CACHE BOOL "Enable parallel build (requires MPI)")
	      set(FLEUR_USE_HDF5MPI TRUE)
	   else()
	      set(HDF5_ENABLE_PARALLEL OFF CACHE BOOL "Enable parallel build (requires MPI)")
	      set(FLEUR_USE_HDF5MPI FALSE)
	   endif()
	   message("FLEUR_USE_HDF5MPI: ${FLEUR_USE_HDF5MPI}")
...
if (FLEUR_USE_HDF5)
   ...
   if (FLEUR_USE_HDF5MPI)
   if (FLEUR_USE_MPI)
      set(FLEUR_PARALLEL_DEFINITIONS ${FLEUR_PARALLEL_DEFINITIONS} "CPP_HDFMPI")
   endif()
   endif()
endif()
