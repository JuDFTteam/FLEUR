       ...
       DO ilSpinPr=MERGE(1,isp,noco%l_noco),MERGE(2,isp,noco%l_noco)
          ...
          DO ilSpin=ilSpinPr,MERGE(2,isp,noco%l_noco)
            IF (.NOT.noco%l_noco) THEN
              ...
              CALL hsmt_nonsph(n,fmpi,sym,atoms,ilSpinPr,ilSpin,1,1,chi_one,noco,nococonv,cell,lapw,td,fjgj,hmat(1,1),.FALSE.,.FALSE.,1)
              CALL hsmt_lo(input,atoms,sym,cell,fmpi,noco,nococonv,lapw,usdus,td,fjgj,n,1,chi_one,ilSpinPr,ilSpin,igSpinPr,igSpin,hmat(1,1),.FALSE.,.FALSE.,.FALSE.,smat=smat(1,1))
            ELSEIF(noco%l_noco.AND..NOT.noco%l_ss) THEN
              ...
              IF (ilSpinPr==ilSpin) THEN !local spin-diagonal contribution
                ...
                CALL hsmt_nonsph(n,fmpi,sym,atoms,ilSpinPr,ilSpinPr,1,1,chi_one,noco,nococonv,cell,lapw,td,fjgj,hmat_tmp,.TRUE.,.FALSE.,1)
                ...
                CALL hsmt_lo(input,atoms,sym,cell,fmpi,noco,nococonv,lapw,usdus,td,fjgj,n,1,chi_one,ilSpinPr,ilSpin,igSpinPr,igSpin,hmat_tmp,.TRUE.,.FALSE.,.FALSE.,smat=smat_tmp)
              ...
              DO igSpinPr=1,2
                DO igSpin=1,2
                  IF (ilSpinPr==ilSpin) THEN !local diagonal spin
                    ...
                    lapw,td,fjgj,hmat(igSpinPr,igSpin),.FALSE.,.FALSE.,1)
                    CALL hsmt_lo(input,atoms,sym,cell,fmpi,noco,nococonv,lapw,usdus,td,fjgj,&
                    n,1,chi(igSpinPr,igSpin),ilSpinPr,ilSpin,igSpinPr,igSpin,hmat(igSpinPr,igSpin),.FALSE.,.FALSE.,.FALSE.,smat=smat(igSpinPr,igSpin))
                  ELSE
