...
MODULE m_hsmt_lo
  ...
  PUBLIC hsmt_lo
CONTAINS
  SUBROUTINE hsmt_lo(Input,Atoms,Sym,Cell,fmpi,Noco,nococonv,Lapw,Ud,Tlmplm,FjGj,N,Chi,ilSpinPr,ilSpin,igSpinPr,igSpin,Hmat,set0,l_pref,Smat,lapwq)
    USE m_hlomat
    USE m_slomat
    USE m_setabc1lo
    USE m_types_mpimat
    ...
    LOGICAL,INTENT(IN)          :: l_pref, set0  !if true, initialize the LO-part of the matrices with zeros
          ...
          IF (atoms%nlo(n).GE.1) THEN
             ...
             !--->       hamiltonian matrix, if they are used for this atom.
               ...
               CALL hlomat(input,atoms,fmpi,lapw,ud,tlmplm,sym,cell,noco,nococonv,ilSpinPr,ilSpin,&
                  n,na,fjgj,alo1,blo1,clo1,igSpinPr,igSpin,chi,hmat,l_pref)
