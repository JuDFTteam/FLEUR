                        ...
                        DO nkvecp = 1,invsfct* (2*lp+1)
                           ...
                           IF (smat%l_real) THEN
                              smat%data_r(lorow,locol) = smat%data_r(lorow,locol) &
                                                     ...
                                                     & * CONJG(cphPr(kp)) * cph(k)
                           ELSE
!                              smat%data_c(lorow,locol) = smat%data_c(lorow,locol) +fact3*merge(cmplx(1.,0.),cmplx(0.,1.),isp==1)
                           
                              smat%data_c(lorow,locol) = smat%data_c(lorow,locol) &
                                                     & + chi * invsfct * fact3 * legpol(atoms%llo(lo,ntyp),dotp) &
                                                     & * CONJG(cphPr(kp)) * cph(k)
                              if (ntyp==1.and.(lo==2.or.lo==3).and.(lop==2.or.lop==3)) &
                                 write(*,"(9(i0,1x),20(e18.11,1x))") lo,lop,lorow,locol,isp,igspin,igspinpr,k,kp,chi,legpol(atoms%llo(lo,ntyp),dotp),fact3,chi * invsfct * fact3 * legpol(atoms%llo(lo,ntyp),dotp) &
                                 & * CONJG(cphPr(kp)) * cph(k)
                            END IF
                        END DO
                     END IF
