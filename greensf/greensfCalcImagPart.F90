MODULE m_greensfCalcImagPart
   ...
   SUBROUTINE greensfCalcImagPart_single_kpt(ikpt,ikpt_i,ev_list,spin_ind,gfinp,atoms,input,kpts,noco,mpi,&
      ...
      INTEGER  :: nBands,jsp,i_gf,nLO,imatSize
            ...
            DO m = -l, l
               ...
               DO iBand = 1, nBands
                  ...
                  IF(eGrid_start==eGrid_end) THEN
                     ...
                     IF(l_sphavg) THEN
                        imag(ie,1) = imag(ie,1) + weight * greensfBZintCoeffs%sphavg(iBand,m,mp,i_elem)
                     ELSE
                        imag(ie,1) = imag(ie,1) + weight * greensfBZintCoeffs%uu(iBand,m,mp,i_elem)
                        imag(ie,2) = imag(ie,2) + weight * greensfBZintCoeffs%dd(iBand,m,mp,i_elem)
                        imag(ie,3) = imag(ie,3) + weight * greensfBZintCoeffs%ud(iBand,m,mp,i_elem)
                        imag(ie,4) = imag(ie,4) + weight * greensfBZintCoeffs%du(iBand,m,mp,i_elem)
                        IF(nLO>0) THEN
                           imat = 0
                           DO iLO = 1, nLO
                              imat = imat + 4
                              imag(ie,imat+1) = imag(ie,imat+1) + weight * greensfBZintCoeffs%uulo(iBand,m,mp,iLO,i_elemLO)
                              imag(ie,imat+2) = imag(ie,imat+2) + weight * greensfBZintCoeffs%ulou(iBand,m,mp,iLO,i_elemLO)
                              imag(ie,imat+3) = imag(ie,imat+3) + weight * greensfBZintCoeffs%dulo(iBand,m,mp,iLO,i_elemLO)
                              imag(ie,imat+4) = imag(ie,imat+4) + weight * greensfBZintCoeffs%ulod(iBand,m,mp,iLO,i_elemLO)
                           ENDDO
                           imat = 0
                           DO iLO = 1, nLO
                              DO iLOp = 1, nLO
                                 imat = imat + 1
                                 imag(ie,4 + 4*nLO+imat) = imag(ie,4 + 4*nLO+imat) + weight * greensfBZintCoeffs%uloulop(iBand,m,mp,imat,i_elemLO)
                              ENDDO
                           ENDDO
                        ENDIF
                     ENDIF
                           ...
                           DO iLO = 1, nLO
                              DO iLOp = 1, nLO
                                 ...
                                 imag(eGrid_start:eGrid_end,4 + 4*nLO+imat) = imag(eGrid_start:eGrid_end,4 + 4*nLO+imat) + weights(eGrid_start:eGrid_end,iBand) &
                                                                              * greensfBZintCoeffs%uloulop(iBand,m,mp,imat,i_elemLO)
                              ENDDO
                           ENDDO
                        ENDIF
                     ENDIF
