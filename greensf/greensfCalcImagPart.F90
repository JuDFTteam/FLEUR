            ...
            DO mp = -lp, lp
               DO m = -l, l
                  ...
                  IF(gfinp%l_resolvent) THEN
                     IF(l_sphavg) THEN
                        CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                   greensfBZintCoeffs%sphavg(:nBands,m,mp,i_elem,ikpt_i,spin_ind),nBands,imag(:,1),1)
                     ELSE
                        CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                   greensfBZintCoeffs%uu(:nBands,m,mp,i_elem,ikpt_i,spin_ind),nBands,imag(:,1),1)
                        CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                   greensfBZintCoeffs%dd(:nBands,m,mp,i_elem,ikpt_i,spin_ind),nBands,imag(:,2),1)
                        CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                   greensfBZintCoeffs%ud(:nBands,m,mp,i_elem,ikpt_i,spin_ind),nBands,imag(:,3),1)
                        CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                   greensfBZintCoeffs%du(:nBands,m,mp,i_elem,ikpt_i,spin_ind),nBands,imag(:,4),1)
                        IF(nLO>0) THEN
                           imat = 0
                           DO iLO = 1, nLO
                              imat = imat + 4
                              CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                         greensfBZintCoeffs%uulo(:nBands,m,mp,iLO,i_elemLO,ikpt_i,spin_ind),nBands,imag(:,imat+1),1)
                              CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                         greensfBZintCoeffs%ulou(:nBands,m,mp,iLO,i_elemLO,ikpt_i,spin_ind),nBands,imag(:,imat+2),1)
                              CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                         greensfBZintCoeffs%dulo(:nBands,m,mp,iLO,i_elemLO,ikpt_i,spin_ind),nBands,imag(:,imat+3),1)
                              CALL zgemm('N','N',SIZE(eMesh),1,nBands,CMPLX(1.0,0.0),weights,SIZE(eMesh),&
                                         greensfBZintCoeffs%ulod(:nBands,m,mp,iLO,i_elemLO,ikpt_i,spin_ind),nBands,imag(:,imat+4),1)
                           ENDDO
                           imat = 0
                           DO iLO = 1, nLO
                              DO iLOp = 1, nLO
