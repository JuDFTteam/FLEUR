...
SUBROUTINE greensfRes21(atoms,sym,angle,input,nbands,resWeights,ind,wtkpt,eig,denCoeffsOffDiag,eigVecCoeffs,greensf)
   ...
   DO i_gf = 1, atoms%n_gf
      ...
      DO natom = SUM(atoms%neq(:nType-1)) + 1, SUM(atoms%neq(:nType))
         ...
         !$OMP SHARED(natom,l,nType,angle,wtkpt,i_gf,nbands,l_tria) &
         !$OMP SHARED(atoms,g,input,eigVecCoeffs,greensf,eig,denCoeffsOffDiag,resWeights,ind) &
                        ...
                        IF(atoms%llo(ilo,nType).EQ.l) THEN
                           ...
                           DO ilop = 1, atoms%nlo(nType)
                              IF(atoms%llo(ilop,nType).EQ.l) THEN
                                 g(iz,m,mp,1) = g(iz,m,mp,1) + weight * denCoeffsOffDiag%uloulop21n(ilo,ilop,nType) *&
...
END SUBROUTINE greensfRes21



END MODULE m_greensfRes21