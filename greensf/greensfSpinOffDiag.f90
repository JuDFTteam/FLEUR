      ...
      DO m = -l, l
         DO mp = -lp,lp
            ...
            DO ilo = 1, atoms%nlo(atomType)
               ...
               ELSE
                  nLO_ind = nLO_ind + 1
                  im(m,mp,:,4+(nLO_ind-1)*2+1) = im(m,mp,:,4+(nLO_ind-1)*2+1) + conjg(eigVecCoeffs%acof(  :nBands,lmp,natomp,spin1))&
                                                                                    * eigVecCoeffs%ccof(m,:nBands,ilo,natom ,spin2)
                  im(m,mp,:,4+(nLO_ind-1)*2+2) = im(m,mp,:,4+(nLO_ind-1)*2+2) + conjg(eigVecCoeffs%bcof(  :nBands,lmp,natomp,spin1))&
                                                                                    * eigVecCoeffs%ccof(m,:nBands,ilo,natom ,spin2)
               ENDIF
            ENDDO
            IF(.NOT.l_sphavg) THEN
               DO ilo = 1, atoms%nlo(atomTypep)
                  ...
                  nLO_ind = nLO_ind + 1
                  im(m,mp,:,4+(nLO_ind-1)*2+1) = im(m,mp,:,4+(nLO_ind-1)*2+1) + conjg(eigVecCoeffs%ccof(mp,:nBands,ilo,natomp,spin1))&
                                                                                    * eigVecCoeffs%acof(   :nBands,lm ,natom ,spin2)
                  im(m,mp,:,4+(nLO_ind-1)*2+2) = im(m,mp,:,4+(nLO_ind-1)*2+2) + conjg(eigVecCoeffs%ccof(mp,:nBands,ilo,natomp,spin1))&
                                                                                    * eigVecCoeffs%bcof(   :nBands,lm ,natom ,spin2)
               ENDDO
            ENDIF
            DO ilo = 1, atoms%nlo(atomType)
               ...
               DO ilop = 1, atoms%nlo(atomTypep)
                  ...
                  ELSE
                     nLOp_ind = nLOp_ind + 1
                     im(m,mp,:,4+nLO_ind*2+nLOp_ind) = im(m,mp,:,4+nLO_ind*2+nLOp_ind) + conjg(eigVecCoeffs%ccof(mp,:nBands,ilop,natomp,spin1))&
                                                                                             * eigVecCoeffs%ccof(m ,:nBands,ilo ,natom ,spin2)
                  ENDIF
               ENDDO
            ENDDO
