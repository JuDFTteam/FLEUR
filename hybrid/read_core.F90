...
CONTAINS
   ...
   SUBROUTINE read_core(atoms, hybdat)
     ...
     &          j_qn(0:ncst, atoms%ntype), stat=ok)
      IF (ok .ne. 0) call judft_error('mhsfock: failure allocation n_qn,l_qn,j_qn')
      ALLOCATE (nindxcr(0:ncst, atoms%ntype), stat=ok)
      IF (ok .ne. 0) call judft_error('mhsfock: failure allocation nindxcr')
