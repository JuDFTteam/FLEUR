         ...
         DO itype = 1, fi%atoms%ntype
            ...
            DO ieq = 1, fi%atoms%neq(itype)
               ...
               IF (iatom1 /= iatom2) THEN
                  ...
                  !$acc data copyin(cmt_nk, cmt_nkqpt)
...
#ifdef _OPENACC
                                          !$acc kernels default(none) present(rarr3, hybdat, hybdat%nbands, cmt_nkqpt, cmt_nk)
#else
                                          ...
                                          DO iband = 1, hybdat%nbands(ik,jsp)
                                             DO ibando = bandoi,bandof
                                                rarr3(1, ibando, iband) = rarr3(1, ibando, iband) &
                                                                        + rdum*cmt_nk(iband, lmp1, iatom1)*cmt_nkqpt(ibando, lmp2, iatom1) &
                                                                        + rdum*cmt_nk(iband, lmp1, iatom2)*cmt_nkqpt(ibando, lmp2, iatom2)

                                                rarr3(2, ibando, iband) = rarr3(2, ibando, iband) &
                                                                        + rdum*cmt_nk(iband, lmp1, iatom1)*cmt_nkqpt(ibando, lmp2, iatom2) &
                                                                        - rdum*cmt_nk(iband, lmp1, iatom2)*cmt_nkqpt(ibando, lmp2, iatom1)
                                    ...
                                    IF (abs(m2) <= l2 .and. offdiag) THEN
                                       ...
                                       IF (abs(rdum) > 1e-12) THEN
                                          ...
                                          DO iband = 1, hybdat%nbands(ik,jsp)
                                             DO ibando = bandoi,bandof
                                                rarr3(1, ibando, iband) = rarr3(1, ibando, iband) &
                                                                        + rdum*cmt_nk(iband, lmp2, iatom1)*cmt_nkqpt(ibando, lmp1, iatom1) &
                                                                        + rdum*cmt_nk(iband, lmp2, iatom2)*cmt_nkqpt(ibando, lmp1, iatom2)

                                                rarr3(2, ibando, iband) = rarr3(2, ibando, iband) &
                                                                        + rdum*cmt_nk(iband, lmp2, iatom1)*cmt_nkqpt(ibando, lmp1, iatom2) &
                                                                        - rdum*cmt_nk(iband, lmp2, iatom2)*cmt_nkqpt(ibando, lmp1, iatom1)
                                             END DO  !ibando
                                          END DO  !iband
#ifdef _OPENACC
                                          !$acc end kernels
                     ...
                     END DO !l
                  !$acc end data ! hybdat, hybdat%nbands, hybdat%prodm, mpdata, mpdata%num_radbasfn
                  ...
                  DO l = 0, fi%hybinp%lcutm1(itype)
                     ...
                     DO n = 1, hybdat%nindxp1(l, itype)
                        ...
                        DO m = -l, -1
                           ...
                           !$acc data copyin(rarr2)
#ifdef _OPENACC
                              !$acc parallel loop default(none) private(j, iob) collapse(3)&
                              !$acc present(cprod, hybdat, hybdat%nbands, hybdat%prodm, mpdata, mpdata%num_radbasfn)
#else
                                 ...
                                 DO ibando = bandoi,bandof
                                    DO i = 1, mpdata%num_radbasfn(l, itype)
                                       ...
                                       cprod(j, iob + (iband-1)*psize) &
                                          = cprod(j, iob + (iband-1)*psize) + hybdat%prodm(i, n, l, itype)*rarr2(ibando, iband)
                        ...
                        !$acc data copyin(rarr2)
#ifdef _OPENACC
                           !$acc parallel loop private(iob) collapse(3)&
                           !$acc present(hybdat, hybdat%nbands, hybdat%prodm, mpdata, mpdata%num_radbasfn, cprod)
#else
                           !$OMP parallel do default(none) private(iband, ibando, iob) collapse(3) &
                           !$OMP shared(hybdat, ik, jsp, bandoi, bandof, mpdata, cprod, lm1, psize, itype, l, rarr2, n)
#endif
                           DO iband = 1, hybdat%nbands(ik,jsp)
                              DO ibando = bandoi,bandof
                                 do j = 1, mpdata%num_radbasfn(l, itype)
                                    iob = ibando + 1 - bandoi
                                    cprod(lm1+j, iob + (iband-1)*psize) = cprod(lm1+j, iob + (iband-1)*psize) &
                                       + rarr2(ibando, iband) * hybdat%prodm(j,n,l,itype)
                                 enddo
                              END DO  !ibando
                           END DO  !iband
#ifdef _OPENACC
                        ...
                        DO m = 1, l
                           ...
                           !$acc data copyin(rarr2)
#ifdef _OPENACC
                              !$acc parallel loop default(none) private(iob) collapse(3)&
                              !$acc present(hybdat, hybdat%nbands, hybdat%prodm, cprod, mpdata, mpdata%num_radbasfn, rarr2)
#else
