...
CONTAINS
  SUBROUTINE process_lattice(line,a1,a2,a3,aa,scale,mat)
    ...
    REAL, PARAMETER :: eps = 1.0e-7, sqrt2  =  1.4142135623730950, &
         sqrt3  =  1.7320508075688773, sqrt32 =  0.86602540378444, msqrt32 = -0.86602540378444
    ...
    ELSEIF (latsys =='hexagonal-R2'.OR.latsys =='hR2'.OR.latsys =='r2'&
         .OR.latsys =='R2'.OR.latsys =='rhombohedral2'.OR. latsys =='trigonal2' ) THEN
    ...
    ELSEIF ( latsys =='orthorhombic-A'.OR.latsys =='oA'.OR.&
         latsys =='orA'.OR. latsys =='base-centered-orthorhombic2' ) THEN
    ...
    ELSEIF ( latsys =='orthorhombic-B'.OR.latsys =='oB'.OR.&
         latsys =='orB'.OR. latsys =='base-centered-orthorhombic3' ) THEN
    ...
    ELSEIF ( latsys =='monoclinic-C'.OR.latsys =='mC'.OR.latsys =='moC'.OR.&
       ...
       CALL juDFT_error("Monoclinic setup",hint="Please use gamma as monoclinic angle"//&
            " and center on A or B !")

       !===> 13a monoclinic-A (mA)
    ELSEIF ( latsys =='monoclinic-A'.OR.latsys =='mA'.OR.latsys =='moA'.OR.&
       ...
       IF ( (abs(alpha-90.0)<eps).AND.(abs(beta-90.0)<eps) ) THEN
          IF ( abs(gamma - 90.0) <eps )  CALL juDFT_error("no monoclinic angle!",calledby ="lattice2")
       ELSE
          CALL juDFT_error("Please take gamma as monoclinic angle!",calledby ="lattice2")
       ENDIF
       CALL brvmat ( alpha, beta, gamma, am )
       am(:,1) = a * am(:,1)
       am(:,2) = b * am(:,2)
    ...
    ELSEIF ( latsys =='monoclinic-I'.OR.latsys =='mI'.OR.latsys =='moI' ) THEN
       ...
       noangles=.false.
       IF ( (abs(alpha-90.0)<eps).AND.(abs(beta-90.0)<eps) ) THEN
          IF ( ABS(gamma - 90.0) <eps )  CALL juDFT_error("no monoclinic angle!")
       ELSE
          CALL juDFT_error("Please take gamma as monoclinic angle!",calledby ="lattice2")
       ENDIF
       CALL brvmat ( alpha, beta, gamma, am )
    ...
    IF ( noangles .AND. ( abs(alpha - 90.0 ) > eps .OR.&
       ...
       call judft_error("ERROR in &lattice ... For the given lattice system all angles should be 90deg.")
    ENDIF

    IF (abs(scale(1)) < eps) THEN
       CALL juDFT_error("Illegal program section ")
    ENDIF
    aa = a0
...
911 CONTINUE
    ...
    RETURN
  END SUBROUTINE process_lattice
  !
  !**********************************************************************!
  !
...
END MODULE m_process_lattice_namelist
