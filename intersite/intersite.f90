MODULE m_intersite
   ...
   SUBROUTINE intersite_coeffs(kpt,atoms,sym,ispin,jspins,noccbd,tetweights,wtkpt,eig,usdus,eigVecCoeffs,greensfCoeffs)
      ...
      INTEGER  i_gf,ntype,n,np,i_np,n_nearest,nn,l,lp,lm,lmp,m,mp,j
      REAL     wk
      LOGICAL  l_zero
      ...
      !Loop through the gf elements to be calculated (for now we use n and n' as arguments)
      DO i_gf = 1, gf%n_gf 
         !atom Type where to calculate the intersite gf
         ntype = gf%atomType(i_gf)
         n = SUM(atoms%neq(:ntype-1))
         DO nn = 1, atoms%neq(ntype)
            n = n+1
            !Find the nearest neighbours to the current atom (TODO)
            !CALL nearest_neigh(n,atoms,nearest,n_nearest)
            n_nearest = 1
            DO i_np = 1, n_nearest
               !TO BE CHANGED TO
               !np = nearest(i_np)
               np = n
               !
               !Calculate the bloch phase:
               !
               phase = exp(ImagUnit*dot_product(kpt,atoms%taual(:,n)-atoms%taual(:,np)))
                           ...
                           DO mp = -lp,lp
                              ...
                              IF(greensfCoeffs%l_tetra) THEN
                                 ...
                                 DO j = 1, greensfCoeffs%ne
                                    ...
                                    greensfCoeffs%uu(j,n,np,lm,lmp,ispin) = greensfCoeffs%uu(j,n,np,m,mp,ispin) - fac * dos_weights(j) * pi_const * REAL(tmp)
                                    
                                    tmp = cil * phase * conjg(eigVecCoeffs%bcof(i,lm,n,ispin))*eigVecCoeffs%bcof(i,lmp,np,ispin)                                                                    
                                    greensfCoeffs%dd(j,n,np,lm,lmp,ispin) = greensfCoeffs%dd(j,n,np,m,mp,ispin) - fac * dos_weights(j) * pi_const * REAL(tmp)
                                    
                                    tmp = cil * phase * conjg(eigVecCoeffs%acof(i,lm,n,ispin))*eigVecCoeffs%bcof(i,lmp,np,ispin)                                                                
                                    greensfCoeffs%du(j,n,np,lm,lmp,ispin) = greensfCoeffs%du(j,n,np,m,mp,ispin) - fac * dos_weights(j) * pi_const * REAL(tmp)
                                                                                                   
                                    tmp = cil * phase * conjg(eigVecCoeffs%bcof(i,lm,n,ispin))*eigVecCoeffs%acof(i,lmp,np,ispin)
                                    greensfCoeffs%ud(j,n,np,lm,lmp,ispin) = greensfCoeffs%ud(j,n,np,m,mp,ispin) - fac * dos_weights(j) * pi_const * REAL(tmp)
                                 ENDDO
                              ELSE
                                 tmp = cil * phase * conjg(eigVecCoeffs%acof(i,lm,n,ispin))*eigVecCoeffs%acof(i,lmp,np,ispin)
                                 greensfCoeffs%uu(j,n,np,lm,lmp,ispin) = greensfCoeffs%uu(j,n,np,m,mp,ispin) - fac * wk * pi_const * REAL(tmp)
                                 
                                 tmp = cil * phase * conjg(eigVecCoeffs%bcof(i,lm,n,ispin))*eigVecCoeffs%bcof(i,lmp,np,ispin)                                                                    
                                 greensfCoeffs%dd(j,n,np,lm,lmp,ispin) = greensfCoeffs%dd(j,n,np,m,mp,ispin) - fac * wk * pi_const * REAL(tmp)
                                 
                                 tmp = cil * phase * conjg(eigVecCoeffs%acof(i,lm,n,ispin))*eigVecCoeffs%bcof(i,lmp,np,ispin)                                                                
                                 greensfCoeffs%du(j,n,np,lm,lmp,ispin) = greensfCoeffs%du(j,n,np,m,mp,ispin) - fac * wk * pi_const * REAL(tmp)
   ...
   SUBROUTINE prepare_intersite(atoms,vr,jspins,gf,greensfCoeffs,sym,lmax)
