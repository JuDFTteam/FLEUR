...
contains
  ...
  SUBROUTINE open_hybrid_io2(mpbasis, hybrid,DIMENSION,atoms,l_real)
    ...
    irecl_coulomb = ( atoms%ntype*(maxval(hybrid%lcutm1)+1)*(maxval(hybrid%num_rad_bas_fun)-1)**2&
         +   atoms%nat *(maxval(hybrid%lcutm1)+2)*(2*maxval(hybrid%lcutm1)+1)*(maxval(hybrid%num_rad_bas_fun)-1)&
         +   (maxval(hybrid%num_rad_bas_fun)-1)*atoms%nat**2&
