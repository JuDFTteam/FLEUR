...
!
 ...
 7121    FORMAT (5x,i1,5x,l1,8x,i1,8x,i1,6x,d10.3)
      ...
      ELSEIF ( ch_test .eq. 'gcu' ) then              ! HF
        BACKSPACE (5)
        READ (UNIT=5,FMT=7999,END=99,ERR=99) hybrid%gcutm,hybrid%tolerance1,&
     &     hybrid%ewaldlambda,hybrid%lexp,hybrid%bands1
        WRITE (6,9999) hybrid%gcutm,hybrid%tolerance1,hybrid%ewaldlambda,hybrid%lexp,hybrid%bands1
...
!
 ...
 9121    FORMAT ('igrd=',i1,',lwb=',l1,',ndvgrd=',i1,',idsprs=',i1,&
     ...
     &    .OR. namex.EQ.'vhse' ) THEN
        WRITE (5,9999) hybrid%gcutm,hybrid%tolerance1,hybrid%ewaldlambda,hybrid%lexp,hybrid%bands1
        l_hyb = .true.
      END IF
