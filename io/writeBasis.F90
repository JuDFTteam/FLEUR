...
!            END DO
		CALL abcof(input,atoms,sym,cell,lapw,numbands,usdus,noco,nococonv,jsp,oneD,&
		    eigVecCoeffs%acof2(:,0:,0,:,jsp),eigVecCoeffs%acof2(:,0:,1,:,jsp),&
...
!            END DO
		CALL abcrot(atoms%ntype,atoms%nat,numbands,atoms%lmaxd,atoms%lmaxd*(atoms%lmaxd+2),atoms%llod,atoms%nlod,atoms%ntype,atoms%neq,&
		           ...
		           oneD%odi,oneD%ods,&
		           eigVecCoeffs%acof2(:,0:,0,:,jsp),eigVecCoeffs%acof2(:,0:,1,:,jsp),eigVecCoeffs%ccof(-atoms%llod:,:,:,:,jsp))
...
!-------------------------output spex format-------------------
		...
		allocate ( cof(Hdim1(1),Hdim1(2),Hdim1(3),Hdim1(4)) )
			...
			do nn = 1,atoms%neq(n)
			  ...
			  do l = 0,atoms%lmax(n)
			    do m = -l,l
			      cof(1,lmn+1,na,:) = real ( eigVecCoeffs%acof2(:numbands,lm,0,na,jsp) * img**l )
			      cof(1,lmn+2,na,:) = real ( eigVecCoeffs%acof2(:numbands,lm,1,na,jsp) * img**l )
			      cof(2,lmn+1,na,:) = aimag ( eigVecCoeffs%acof2(:numbands,lm,0,na,jsp) * img**l )
			      cof(2,lmn+2,na,:) = aimag ( eigVecCoeffs%acof2(:numbands,lm,1,na,jsp) * img**l )
			      lm  = lm  + 1
			      lmn = lmn + 2
			      do i = 1,atoms%nlo(n)
