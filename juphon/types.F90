...
! General potential type for perturbed and unperturbed potential
type :: t_potential
    real,     allocatable     ::  vr(:,:,:,:)    ! radial potential in MT sphere
    real,     allocatable     ::  vr0( :,:,:)    ! averaged MT potential multiplied with r*/sqrt(4*pi)
    real,     allocatable     ::  vz(:,:,:)      ! interstitial in stars entwickelt
    complex,  allocatable     ::  vpw(:,:)
    complex,  allocatable     ::  vpw_uw(:, :)
    complex,  allocatable     ::  vzxy(:,:,:,:)
...
!
     ...
     TYPE t_input
        ...
        REAL    :: epsdisp !< minimal displacement. If all displacements are < epsdisp NOstopNO
        REAL    :: epsforce !< minimal force. If all forces <epsforce NOstopNO
        INTEGER :: isec1
        INTEGER :: krla
        REAL    :: delgau
        REAL    :: alpha
