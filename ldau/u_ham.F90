...
MODULE m_uham
  ...
  !-------------------------------------------------------------------+
CONTAINS
  SUBROUTINE u_ham(atoms, input,lapw,jsp,n,i_u,invsfct,&
    ...
    !     ..
    !     .. Local Scalars ..
    INTEGER  m,mp,ispin,itype,l,igp,ll,llm,nvv,ii,lp,lop,ig
    ...
    DO m = -l,l
       DO mp = -l,l
          va_help(mp) = REAL(vs_mmp(m,mp,i_u,jsp)) * invsfct
       ...
       DO mp = -l,l
          va_help(mp) = AIMAG(vs_mmp(m,mp,i_u,jsp)) * invsfct
          vb_help(mp) = va_help(mp) * ud%ddn(l,n,jsp)
       END DO
    ...
    DO ig = n_rank+1, nvv, n_size
       ...
       IF (noco%l_ss) THEN
          ...
          IF ((iintsp.EQ.1).AND.(jintsp.EQ.1)) THEN     !---> spin-up spin-up part
             ...
             chihlp = chi11
             ii = (ig-1)*(ig)/2
          ELSE IF ((iintsp.EQ.2).AND.(jintsp.EQ.2)) THEN !---> spin-down spin-down part
             igp_m = ig
             chihlp = chi22
    ...
    DO lo = 1, atoms%nlo(n)
       IF (l.EQ.atoms%llo(lo,n)) THEN
          ...
          ALLOCATE (cx(-l:l,2*(2*atoms%llod+1)),we_help(-l:l), wa_help(-l:l),wb_help(-l:l))
       ...
       ELSE
          ...
          locol = locol + lp
       END IF  ! end of if(l=atoms%llo(lo,n))
