...
SUBROUTINE stden(mpi,sphhar,stars,atoms,sym,vacuum,&
   ...
   IF (input%jspins.EQ.2 .AND. MAXVAL(ABS(atoms%econf(:)%occupation(:,1)-atoms%econf(:)%occupation(:,2))).EQ.0 ) juDFT_warn("You're setting up a spin-polarized calculation (jspins=2) without any acutal polarization given in the systems occupation. You're sure you want that?", calledby = "stden")
