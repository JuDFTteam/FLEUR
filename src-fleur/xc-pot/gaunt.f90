...
!*********************************************************************
   ...
   REAL,SAVE,ALLOCATABLE::w2(:),yr2(:,:)
   PUBLIC gaunt1, gaunt2, gaunt_init, gaunt_init2
CONTAINS
...
! heck if the previous call of the subroutine was with the same lmaxd
   ...
   FUNCTION gaunt2(lp,l,ls,mp,m,ms,lmaxd)
      ...
      n= (3*lmaxd)/4+1
! heck if this is first call to subroutine
      IF(.NOT. ALLOCATED(YR2)) CALL gaunt_init2(lmaxd)
! heck if the previous call of the subroutine was with the same lmaxd
      IF(lmaxd > lmaxdp2) call juDFT_error("Can't calc gaunt. lmaxd too high")
      ...
      gaunt2 = dot_product(w2, yr2(:,ilp)*yr2(:,il)*yr2(:,ils))
   END FUNCTION
...
!--->    generate associated legendre functions for m.ge.0
   ...
   SUBROUTINE gaunt_init2(lmaxd)
      ...
      if (allocated(w2)) return
      n = (3*lmaxd)/4+1
      ALLOCATE(w2(n),  source=0.0)
      ALLOCATE(yr2(n,(lmaxd+1)**2), source=0.0)
      lmaxdp2 = lmaxd
      lmax1d = lmaxd+1
      ...
      DO  k = 1,n
         ...
         DO  l = 0,lomax
            ...
            DO  m = 1,l
               t = (l+1-m)* (l+m)
               cd = cd/t
               yr2(k,lm+m) = a*sqrt(cd)*p(l,m)
               yr2(k,lm-m) = sgm*a*sqrt(cd)*p(l,m)
               sgm = -sgm
            ENDDO
         ENDDO
      ENDDO
