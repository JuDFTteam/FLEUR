...
CONTAINS
    SUBROUTINE dfpt_check(fi, xcpot)
      ...
      IF (l_xc) CALL judft_error("You compiled Fleur without libxc but want to use DFPT. Please fix that.",calledby="dfpt_check.F90")
        ...
        !DFTU etc.
        IF ((fi%atoms%n_u.GT.0).OR.(fi%atoms%n_hia.GT.0).OR.(fi%atoms%n_opc.GT.0)) CALL judft_error("Currently dfpt doesn't support DFT+X.",calledby="dfpt_check.F90")

        !SOC:
        IF (fi%noco%l_soc) CALL judft_error("dfpt doesn't support spin-orbit coupling.",calledby="dfpt_check.F90")

        !Spin spirals:
        IF (fi%noco%l_ss) CALL judft_error("dfpt doesn't support spin-spiral systems.",calledby="dfpt_check.F90")

        !vdW
        IF (fi%input%vdw.GT.0) CALL judft_error("dfpt doesn't contain van-der-Waals corrections.",calledby="dfpt_check.F90")

        !Hybrid/RDMFT
        IF (fi%hybinp%l_hybrid .OR. fi%input%l_rdmft) CALL judft_error("dfpt doesn't support hybrid or RDMFT.",calledby="dfpt_check.F90")
