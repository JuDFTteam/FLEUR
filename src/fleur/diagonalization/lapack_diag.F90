!--------------------------------------------------------------------------------
! Copyright (c) 2016 Peter Grünberg Institut, Forschungszentrum Jülich, Germany
! This file is part of FLEUR and available as free software under the conditions
! of the MIT license as expressed in the LICENSE file in more detail.
!--------------------------------------------------------------------------------
MODULE m_lapack_diag
  USE m_types_mat
  USE m_judft 
  USE omp_lib
IMPLICIT NONE
  CONTAINS
#if 1==1
  SUBROUTINE lapack_diag(hmat,smat,ne,eig,zmat)
    !Simple driver to solve Generalized Eigenvalue Problem using LAPACK routine
    IMPLICIT NONE
    TYPE(t_mat),INTENT(INOUT)  :: hmat,smat
    INTEGER,INTENT(INOUT)      :: ne
    CLASS(t_mat),ALLOCATABLE,INTENT(OUT)    :: zmat
    REAL,INTENT(OUT)           :: eig(:)

    INTEGER            :: lwork,info,m
    INTEGER,ALLOCATABLE:: ifail(:),iwork(:)
    COMPLEX,ALLOCATABLE:: work(:)
    REAL,ALLOCATABLE   :: rwork(:)
    REAL               :: dumrwork(1),abstol
    COMPLEX            :: dumwork(1)
    REAL,external      :: dlamch
    REAL               :: eigTemp(hmat%matsize1)

    
    ALLOCATE(t_mat::zmat)
    CALL zmat%alloc(hmat%l_real,hmat%matsize1,ne)
    abstol=2*dlamch('S')
    IF (hmat%l_real) THEN
       ALLOCATE(iwork(5*hmat%matsize1),ifail(hmat%matsize1))
       CALL dsygvx(1,'V','I','U', hmat%matsize1,hmat%data_r,SIZE(hmat%data_r,1),smat%data_r,SIZE(smat%data_r,1),&
            0.0,0.0,1,ne,abstol,m,eigTemp,zmat%data_r,SIZE(zmat%data_r,1),dumrwork,-1, iwork, ifail, info)
       lwork=dumrwork(1)
       ALLOCATE(rwork(lwork))
       IF (info.NE.0) CALL judft_error("Diagonalization via LAPACK failed (Workspace)",no=info)
       CALL dsygvx(1,'V','I','U', hmat%matsize1,hmat%data_r,SIZE(hmat%data_r,1),smat%data_r,SIZE(smat%data_r,1),&
            0.0,0.0,1,ne,abstol,m,eigTemp,zmat%data_r,SIZE(zmat%data_r,1),rwork, lwork, iwork, ifail, info)
    ELSE
       ALLOCATE(rwork(7*hmat%matsize1),iwork(5*hmat%matsize1),ifail(hmat%matsize1))
       !Do a workspace query
       CALL zhegvx(1,'V','I','U',hmat%matsize1,hmat%data_c,SIZE(hmat%data_c,1),smat%data_c,SIZE(smat%data_c,1),&
            0.0,0.0,1,ne,abstol,m,eigTemp,zmat%data_c,SIZE(zmat%data_c,1),dumwork,-1,rwork,iwork,ifail,info)
       lwork=dumwork(1)
       ALLOCATE(work(lwork))
       IF (info.NE.0) CALL judft_error("Diagonalization via LAPACK failed (Workspace)",no=info)
       !Perform diagonalization
       CALL zhegvx(1,'V','I','U',hmat%matsize1,hmat%data_c,SIZE(hmat%data_c,1),smat%data_c,SIZE(smat%data_c,1),&
            0.0,0.0,1,ne,abstol,m,eigTemp,zmat%data_c,SIZE(zmat%data_c,1),work,lwork,rwork,iwork,ifail,info)
    ENDIF
    eig(:MIN(SIZE(eig),SIZE(eigTemp))) = eigTemp(:MIN(SIZE(eig),SIZE(eigTemp)))
    IF (info.NE.0) CALL judft_error("Diagonalization via LAPACK failed(zhegvx/dsygvx)",no=info)
    IF (m.NE.ne) CALL judft_error("Diagonalization via LAPACK failed failed without explicit errorcode.")
  END SUBROUTINE lapack_diag
#else
#if 1==2
  SUBROUTINE lapack_diag(hmat,smat,ne,eig,zmat)
    !Simple driver to solve Generalized Eigenvalue Problem using LAPACK routine
    IMPLICIT NONE
    TYPE(t_mat),INTENT(INOUT)  :: hmat,smat
    INTEGER,INTENT(INOUT)      :: ne
    CLASS(t_mat),ALLOCATABLE,INTENT(OUT)    :: zmat
    REAL,INTENT(OUT)           :: eig(:)

    INTEGER            :: info,m,n
    REAL               :: abstol
    REAL,external      :: dlamch
    REAL               :: eigTemp(hmat%matsize1)

    ALLOCATE(t_mat::zmat)
    CALL zmat%alloc(hmat%l_real,hmat%matsize1,ne)
    abstol=2*dlamch('S')
    n=smat%matsize1 !Matrix size
    if (smat%l_real) THEN
      ! Perform Cholesky decomposition of B to obtain L (B = L * L^T)
      call dpotrf('U', n, smat%data_r, n, info)
      if (info /= 0) call juDFT_error("Error in Cholesky decomposition of B")
      
      ! Transform A to A' = L^-1 * A * L^-T using chegst
      call dsygst(1,"U", n, hmat%data_r, n, smat%data_r, n, info)
      if (info /= 0) call juDFT_error("Error in dsygst")

      ! Call LAPACK's cheevr to solve the standard eigenvalue problem A' x = lambda x
      BLOCK  !workspace locally
        integer:: isuppz(2*n),lrwork,liwork
        real   :: rwork_dum
        real,allocatable     :: rwork(:)
        integer,allocatable  :: iwork(:)
        ! Workspace query
        call dsyevr('V', 'I','U', n, hmat%data_r, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_r, n, isuppz, rwork_dum, -1,liwork,-1, info)
        lrwork=rwork_dum
        allocate(rwork(lrwork),iwork(liwork))
        call dsyevr('V', 'I','U', n, hmat%data_r, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_r, n, isuppz,  rwork, lrwork,iwork,liwork, info)
      end block  
      ! Check if the computation was successful
      if (info /= 0) call judft_error("Error in dsyevr")

      ! recover the generalized eigenvectors z by solving z' = l^t * z
      CALL dtrtrs('U','N','N',n,m,smat%data_r,n,zMat%data_r,n,info)
      if (info /= 0) call juDFT_error("Error in back transformation (dpotrs)")
    else
      ! Perform Cholesky decomposition of B to obtain L (B = L * L^T)
      call zpotrf('U', n, smat%data_c, n, info)
      if (info /= 0) call juDFT_error("Error in Cholesky decomposition of B")
      
      ! Transform A to A' = L^-1 * A * L^-T using chegst
      call zhegst(1,"U", n, hmat%data_c, n, smat%data_c, n, info)
      if (info /= 0) call juDFT_error("Error in zhegst")

      ! Call LAPACK's cheevr to solve the standard eigenvalue problem A' x = lambda x
      BLOCK  !workspace locally
        integer:: isuppz(2*n),lwork,lrwork,liwork
        complex:: work_dum
        real   :: rwork_dum
        complex,allocatable  :: work(:)
        real,allocatable     :: rwork(:)
        integer,allocatable  :: iwork(:)
        ! Workspace query
        call zheevr('V', 'I','U', n, hmat%data_c, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_c, n, isuppz, work_dum,-1, rwork_dum, -1,liwork,-1, info)
        lwork=work_dum
        lrwork=rwork_dum
        allocate(work(lwork),rwork(lrwork),iwork(liwork))
        call zheevr('V', 'I','U', n, hmat%data_c, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_c, n, isuppz, work,lwork, rwork, lrwork,iwork,liwork, info)
      end block  
      ! Check if the computation was successful
      if (info /= 0) call judft_error("Error in zheevr")

      ! --> recover the generalized eigenvectors z by solving z' = l^t * z
      CALL ztrtrs('U','N','N',n,m,smat%data_c,n,zMat%data_c,n,info)
      if (info /= 0) call juDFT_error("Error in back transformation (zpotrs)")
    endif
    eig(:MIN(SIZE(eig),SIZE(eigTemp))) = eigTemp(:MIN(SIZE(eig),SIZE(eigTemp)))
END SUBROUTINE lapack_diag
#else
SUBROUTINE lapack_diag(hmat,smat,ne,eig,zmat)
  !Simple driver to solve Generalized Eigenvalue Problem using LAPACK routine
  IMPLICIT NONE
  TYPE(t_mat),INTENT(INOUT)  :: hmat,smat
  INTEGER,INTENT(INOUT)      :: ne
  CLASS(t_mat),ALLOCATABLE,INTENT(OUT)    :: zmat
  REAL,INTENT(OUT)           :: eig(:)

  INTEGER            :: info,m,n
  REAL               :: abstol
  REAL,external      :: dlamch
  REAL               :: eigTemp(hmat%matsize1)

  ALLOCATE(t_mat::zmat)
  CALL zmat%alloc(hmat%l_real,hmat%matsize1,ne)
  abstol=2*dlamch('S')
  n=smat%matsize1 !Matrix size
  if (smat%l_real) THEN
    ! Perform Cholesky decomposition of B to obtain L (B = L * L^T)
    call dpotrf('U', n, smat%data_r, n, info)
    if (info /= 0) call juDFT_error("Error in Cholesky decomposition of B")
    
    ! Transform A to A' = L^-1 * A * L^-T using chegst
    call dsygst(1,"U", n, hmat%data_r, n, smat%data_r, n, info)
    if (info /= 0) call juDFT_error("Error in dsygst")

    ! Call LAPACK's cheevr to solve the standard eigenvalue problem A' x = lambda x
    BLOCK  !workspace locally
      integer:: lrwork
      real   :: rwork_dum
      real,allocatable     :: rwork(:)
      integer:: iwork(5*n),ifail(n)
      ! Workspace query
      call dsyevx('V', 'I','U', n, hmat%data_r, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_r, n, rwork_dum, -1,iwork,ifail, info)
      lrwork=rwork_dum
      allocate(rwork(lrwork))
      call omp_set_num_threads(1)
      
      call dsyevx('V', 'I','U', n, hmat%data_r, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_r, n, rwork, lrwork,iwork,ifail, info)
      call omp_set_num_threads(8)
      
    end block  
    ! Check if the computation was successful
    if (info /= 0) call judft_error("Error in dsyevr")

    ! recover the generalized eigenvectors z by solving z' = l^t * z
    CALL dtrtrs('U','N','N',n,m,smat%data_r,n,zMat%data_r,n,info)
    if (info /= 0) call juDFT_error("Error in back transformation (dpotrs)")
  else
    ! Perform Cholesky decomposition of B to obtain L (B = L * L^T)
    call zpotrf('U', n, smat%data_c, n, info)
    if (info /= 0) call juDFT_error("Error in Cholesky decomposition of B")
    
    ! Transform A to A' = L^-1 * A * L^-T using chegst
    call zhegst(1,"U", n, hmat%data_c, n, smat%data_c, n, info)
    if (info /= 0) call juDFT_error("Error in zhegst")

    ! Call LAPACK's cheevr to solve the standard eigenvalue problem A' x = lambda x
    BLOCK  !workspace locally
      integer:: lwork
      complex:: work_dum
      complex,allocatable  :: work(:)
      real     :: rwork(7*n)
      integer  :: iwork(5*n),ifail(n)
      ! Workspace query
      call zheevx('V', 'I','U', n, hmat%data_c, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_c, n, work_dum,-1, rwork,iwork,ifail, info)
      lwork=work_dum
      allocate(work(lwork))
      call zheevx('V', 'I','U', n, hmat%data_c, n, 0.0, 0.0, 1, ne, abstol, m, eigTemp, zmat%data_c, n,  work,lwork, rwork, iwork,ifail, info)
    end block  
    ! Check if the computation was successful
    if (info /= 0) call judft_error("Error in zheevr")

    ! --> recover the generalized eigenvectors z by solving z' = l^t * z
    CALL ztrtrs('U','N','N',n,m,smat%data_c,n,zMat%data_c,n,info)
    if (info /= 0) call juDFT_error("Error in back transformation (zpotrs)")
  endif
  eig(:MIN(SIZE(eig),SIZE(eigTemp))) = eigTemp(:MIN(SIZE(eig),SIZE(eigTemp)))
END SUBROUTINE lapack_diag
#endif
#endif
END MODULE m_lapack_diag
