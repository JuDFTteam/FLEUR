      ...
      IF (atoms%l_geo(n)) THEN
         ...
         DO natrun = natom, natom + atoms%neq(n) - 1
            ...
            DO ilo = 1, atoms%nlo(n)
               ...
               DO m1 = -l1, l1
                  lm1 = l1*(l1+1) + m1
                  acof_flapw(:,lm1) = acof_flapw(:,lm1) - acoflo(m1,:,ilo,natrun)
                  bcof_flapw(:,lm1) = bcof_flapw(:,lm1) - bcoflo(m1,:,ilo,natrun)
               END DO
            END DO
            ...
            DO it = 1, sym%invarind(natom)
               ...
               isinv = sym%invtab(is)
               vecsum(:) = vecsum(:) + MATMUL(sym%mrot(:, :, isinv), gvint)
            END DO

            ! Transform back to Cartesian coordinates
            starsum = MATMUL(cell%amat, vecsum)
            forc_a12(:) = forc_a12(:) + starsum(:)/sym%invarind(natrun)
