...
CONTAINS
  ...
  SUBROUTINE dmi_init(this,q,theta,phi,ef_shifts,ntype)
    ...
    ALLOCATE(this%evsum(size(this%ef),0:SIZE(this%phi)))
    ALLOCATE(this%h_so(0:ntype,size(this%ef),SIZE(this%phi)))
  ...
  LOGICAL FUNCTION dmi_next_job(this,fmpi,lastiter,atoms,noco,nococonv)
    ...
    IF (fmpi%irank .EQ. 0) THEN
       WRITE(attributes(1),'(a)') 'DMI_SCF'
       CALL openXMLElementPoly('Forcetheorem_Loop',(/'calculationType'/),attributes)
    END IF
  ...
  SUBROUTINE dmi_postprocess(this,fi,results)
    ...
    IF (this%l_io) THEN
       ...
       attributes = ''
       WRITE(attributes(1),'(i5)') SIZE(this%evsum,1)
       WRITE(attributes(2),'(i5)') SIZE(this%evsum,2)-1
       WRITE(attributes(3),'(a)') 'Htr'
       CALL openXMLElement('Forcetheorem_DMI',(/'ef     ','Angles ','units  '/),attributes(:3))
          DO nef=1,size(this%ef,1)
             WRITE(attributes(3),'(f10.5)') this%ef(nef)
             WRITE(attributes(4),'(f15.8)') this%evsum(nef,0)
             CALL writeXMLElementForm('Entry',(/'ef-shift','ev-sum  '/),attributes(3:4),RESHAPE((/1,8,6,5,12,12/),(/3,2/)))
             DO n=1,SIZE(this%evsum,2)-1
                WRITE(attributes(4),'(f15.8)') this%theta(n)
                WRITE(attributes(5),'(f15.8)') this%phi(n)
                WRITE(attributes(6),'(f15.8)') this%evsum(nef,n)
                CALL writeXMLElementForm('Entry',(/'ef-shift','theta   ','phi     ','ev-sum  '/),attributes(3:6),RESHAPE((/1,8,5,3,6,5,20,20,20,20/),(/5,2/)))
                write(attributes(6),'(f15.8)') this%h_so(0,nef,n)
                CALL writeXMLElementForm('allAtoms',(/'ef-shift','theta   ','phi     ','H_so    '/),attributes(3:6),RESHAPE((/1,8,5,3,6,5,20,20,20,20/),(/5,2/)))
                DO i=1,size(this%h_so,1)-1
                   write(attributes(6),'(f15.8)') this%h_so(i,nef,n)
                   write(attributes(2),'(i0)') i
                   CALL writeXMLElementForm('singleAtom',(/'atomType','ef-shift','theta   ','phi     ','H_so    '/),attributes(2:),RESHAPE((/8,8,1,5,3,6,5,5,20,20,20,20/),(/6,2/)))
                END DO
             END DO
          END DO
