...
CONTAINS
   SUBROUTINE stepf_analytical(sym, stars, atoms, input, cell, fmpi, fftgrid, qvec, iDtype, iDir, iOrd, stepf_array)
      ...
      !      END DO
      !   ELSE 
      !      stepf_array(0,:,:) = stepf_array(0,:,:) +  cell%vol*inv_omtil - 1.0
      !      DO z2 = 1, 3*stars%mx3 - 1
      !         gInt(3) = REAL(z2)
      !         IF (gInt(3) > 1.5*stars%mx3) gInt(3) = gInt(3) - 3.0 * stars%mx3
      !         th = cell%bmat(3, 3)*gInt(3)*cell%z1
      !         stepf_array(z2*layerDim,:,:) =  stepf_array(z2*layerDim,:,:) + cell%vol*inv_omtil*SIN(th)/th
      !      END DO
      !   END IF 
      !END IF
...
#ifdef CPP_MPI
         IF (PRESENT(stepf_array)) THEN 
            call MPI_BCAST(stepf_array,size(stepf_array),MPI_DOUBLE_COMPLEX,0,fmpi%mpi_comm,ierr)
         END IF 
#endif 
   END SUBROUTINE

   SUBROUTINE stepf_stars(fmpi,stars,fftgrid,sym,input,atoms,cell,vacuum,qvec)
...
!#endif
      ...
      IF (input%film) then
         ...
         atomsLocal%ntype = 0
         starsLocal = stars
         call stepf(sym,starsLocal,atomsLocal,input,cell,vacuum,fmpi)
         !IF (fmpi%irank==0) call save_npy("vacuum_stepf.npy",starsLocal%ufft) 
      END IF 
      ...
      IF (fmpi%irank == 0 ) THEN 
         ...
         ELSE
            IF (input%film) THEN 
               stars%ufft1=fftgrid%grid * starsLocal%ufft
            ELSE 
