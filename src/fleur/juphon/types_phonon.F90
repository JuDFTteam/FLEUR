...
module m_types_phonon
    ...
    subroutine postprocessing_qpoint_phonon(this,fi,fmpi,juPhon,qpts,iQ,q_list)
        ...
        if (fmpi%irank==0) then 
            ...
            call DiagonalizeDynMat(fi%atoms, qpts%bk(:,q_list(iQ)), juPhon%calcEigenVec, dyn_mat(iQ,:,:), eigenVals, eigenVecs, q_list(iQ),.TRUE.,"raw",juphon%l_sumrule_scf,l_writeOutput=.true.)
