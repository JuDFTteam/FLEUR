            ...
            wk = wtkpt(nk_i) / de
               DO nu = 1 , SIZE(matrix_element,2)
                  DO iNupr = 1 , size(matrix_element,1)
                     ...
                     i = NINT((eig(nu,nk_i,jsp) -eigq(iNupr,nk_i,jsp) -shift - emin)/de) + 1
                     IF ( (i.LE.size(g,1)) .AND. (i.GE.1) ) THEN
         ...
         DO nk_i = 1 , size(matrix_element,3)
            wk = wtkpt(nk_i) / de
               DO nu = 1 , SIZE(matrix_element,2)
                  ! delta(eps_nu(k) - E_F)
                  gridk = 0.0
                  i = NINT((eig(nu,nk_i,jsp) -shift - emin)/de) + 1
                  ...
                  DO iNupr = 1 , size(matrix_element,1)
                     ...
                     gridq = 0.0
                     j = NINT((eigq(iNupr,nk_i,jsp) -shift - emin)/de) + 1
