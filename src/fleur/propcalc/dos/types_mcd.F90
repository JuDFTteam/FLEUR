...
contains
   ...
   subroutine calc_mt_mcd(mcd, banddos, atoms, ev_list, abc, itype, ikpt, jsp)
      ...
      INTEGER, INTENT(IN)   :: itype, ikpt, jsp
            ...
            DO l = 0, 3
               DO m = -l, l
                  ...
                  DO natom = 1, atoms%neq(itype)
                     sumaa = sumaa + abc%cof(i, lm, 1, natom)*CONJG(abc%cof(i, lm, 1, natom))
                     sumbb = sumbb + abc%cof(i, lm, 2, natom)*CONJG(abc%cof(i, lm, 2, natom))
                     sumab = sumab + abc%cof(i, lm, 1, natom)*CONJG(abc%cof(i, lm, 2, natom))
                     sumba = sumba + abc%cof(i, lm, 2, natom)*CONJG(abc%cof(i, lm, 1, natom))
                  END DO
                  DO icore = 1, mcd%ncore(itype)
                     DO ipol = 1, 3
                        index = 3*(n_dos - 1) + ipol
                        mcd%mcd(index, icore, ev_list(i), ikpt, jsp) = mcd%mcd(index, icore, ev_list(i), ikpt, jsp) + &
