...
MODULE m_rixs_io
   ...
   PUBLIC :: rixs_open_contribution_table, rixs_open_spinor_contribution_table
   PUBLIC :: rixs_write_contribution_rows, rixs_write_spinor_contribution_rows, rixs_close_contribution_table
...
CONTAINS
   ...
   SUBROUTINE rixs_write_spinor_contribution_rows(io_unit, ikpt, absorber_atom, absorber_type, eig_band, occupation, &
                                                  k_weight, core_energy, omega_in, gamma_core, matrix_abs, matrix_emit, &
                                                  loss_grid, eta_loss, valence_band_min, valence_band_max, &
      ...
      IF (SIZE(eig_band) /= SIZE(occupation) .OR. SIZE(matrix_abs, 1) < SIZE(eig_band) .OR. &
         ...
         CALL juDFT_error("Inconsistent spinor RIXS contribution-table dimensions", calledby="m_rixs_io")
      END IF
      IF (valence_band_min < 1 .OR. valence_band_max > SIZE(eig_band) .OR. valence_band_min > valence_band_max) THEN
         CALL juDFT_error("Invalid spinor RIXS valence band bounds in contribution table", calledby="m_rixs_io")
      END IF
      IF (intermediate_band_min < 1 .OR. intermediate_band_max > SIZE(eig_band) .OR. &
          intermediate_band_min > intermediate_band_max) THEN
      ...
      IF (l_accumulate_check) THEN
         ...
         END IF
         IF (eta_loss <= 0.0) THEN
            CALL juDFT_error("etaLoss must be positive in spinor RIXS contribution check", calledby="m_rixs_io")
         END IF
      END IF
      ...
      DO band_v = valence_band_min, valence_band_max
         ...
         DO band_n = intermediate_band_min, intermediate_band_max
            ...
            IF (amplitude_abs2 < TINY(amplitude_abs2)) CYCLE
            weight = k_weight*occupation_v*vacancy_n
            IF (weight <= 0.0) CYCLE
            loss_energy = eig_band(band_n) - eig_band(band_v)
            weighted_strength = weight*amplitude_abs2
            IF (l_accumulate_check) THEN
               DO i_grid = 1, SIZE(loss_grid)
                  gaussian = xas_gaussian_broadening(loss_grid(i_grid) - loss_energy, eta_loss)
                  IF (gaussian == 0.0) CYCLE
   ...
   SUBROUTINE rixs_print_setup_summary(rixs, l_noco, l_soc)
      ...
      IF (rixs%rixs_write_contributions) THEN
         IF (l_noco) THEN
            WRITE(*, '(a)') " Contribution amplitudes  : complex coherent amplitudes stored explicitly"
         ELSE
            WRITE(*, '(a)') " Contribution amplitudes  : real/imaginary placeholders; amplitude_abs2 is the S1 spin trace"
         END IF
   ...
   SUBROUTINE rixs_print_contribution_check(pol_in, pol_out, spectrum, contribution_spectrum, l_spinor)
      CHARACTER(LEN=*), INTENT(IN) :: pol_in, pol_out
      REAL,             INTENT(IN) :: spectrum(:), contribution_spectrum(:)
      LOGICAL, OPTIONAL, INTENT(IN) :: l_spinor

      REAL, PARAMETER :: abs_tol = 1.0e-12, rel_tol = 1.0e-10
      REAL :: max_abs_diff, max_rel_diff, norm
      CHARACTER(LEN=4) :: status
      LOGICAL :: l_spinor_check
      ...
      l_spinor_check = .FALSE.
      IF (PRESENT(l_spinor)) l_spinor_check = l_spinor
      IF (l_spinor_check) THEN
         WRITE(*, '(a,a,a)') " RIXS ", TRIM(pol_in)//TRIM(pol_out), " spinor contribution-spectrum check:"
      ELSE
