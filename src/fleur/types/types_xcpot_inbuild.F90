...
!c
      ...
      IF (xcpot%needs_grad()) THEN
         ...
         IF (xcpot%is_name("l91")) THEN    ! local pw91
            CALL vxcl91(jspins,ngrid,ngrid,rh,grad%agrt ,grad%agru ,grad%agrd , grad%g2rt ,&
                 grad%g2ru ,grad%g2rd ,grad%gggrt ,grad%gggru ,grad%gggrd ,&
                 grad%gzgr , vx ,vxc , isprsv,sprsv)
         ELSEIF (xcpot%is_name("pw91")) THEN  ! pw91
            IF (lwbc) THEN
               CALL vxcwb91(jspins,ngrid,ngrid,rh ,grad%agrt ,grad%agru ,grad%agrd ,&
                 grad%g2rt ,grad%g2ru ,grad%g2rd ,grad%gggrt ,grad%gggru ,&
                 grad%gggrd ,grad%gzgr , vx ,vxc , idsprs,isprsv,sprsv)
            ELSE

               CALL vxcpw91(jspins,ngrid,ngrid,rh ,grad%agrt ,grad%agru ,grad%agrd ,&
                 grad%g2rt ,grad%g2ru ,grad%g2rd ,grad%gggrt ,grad%gggru ,&
                 grad%gggrd,grad%gzgr, vx ,vxc , idsprs,isprsv,sprsv)
...
!c
      ...
      IF (xcpot%exc_is_gga()) THEN
         ...
         IF (xcpot%is_name("l91")) THEN  ! local pw91
            CALL excl91(jspins,ngrid,ngrid,rh ,grad%agrt,grad%agru,grad%agrd,grad%g2rt,grad%g2ru,grad%g2rd,grad%gggrt,grad%gggru,grad%gggrd,grad%gzgr, exc, isprsv,sprsv)
         ELSEIF (xcpot%is_name("pw91")) THEN     ! pw91
            IF (lwbc) THEN
               CALL excwb91(ngrid,ngrid,rh( :,1),rh( :,2),grad%agrt,grad%agru,grad%agrd, grad%g2rt,grad%g2ru,grad%g2rd,grad%gggrt,grad%gggru,grad%gggrd,grad%gzgr, exc, idsprs,isprsv,sprsv)
            ELSE
               CALL excpw91(jspins,ngrid,ngrid,rh ,grad%agrt,grad%agru,grad%agrd, grad%g2rt,grad%g2ru,grad%g2rd,grad%gggrt,grad%gggru,grad%gggrd,grad%gzgr, exc, idsprs,isprsv,sprsv)
            ENDIF
         ELSE
            CALL excepbe(xcpot%data,jspins,ngrid,ngrid, rh ,grad%agrt,grad%agru,grad%agrd,grad%g2ru,grad%g2rd,grad%gggrt,grad%gggru,grad%gggrd, exc)
         ENDIF
      ELSE !LDA
         IF (xcpot%is_name("x-a"))  THEN   ! X-alpha method
