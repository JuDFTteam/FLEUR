...
CONTAINS
  SUBROUTINE wannier(&
       fmpi,input,kpts,sym,atoms,stars,vacuum,sphhar ,&
...
!    CALL wann_read_inp(input,l_p0,wann) !Call wann_read_inp in fleur_init
    ...
    IF(wann%l_updown)THEN
       CALL wann_updown(&
            ...
            stars%sk2,stars%phi2 ,fmpi%irank,fmpi%isize,&
            stars%ng3,&
    ...
    IF(wann%l_matrixuHu)THEN
       ...
       CALL wann_uHu(&
            ...
            banddos ,noco,nococonv,cell,vTot,wannTemp,enpara,eig_idList,&
            ...
            atoms%lmax,sym%invsat,sym%invsatnr,nkpt,atoms%taual,&
            atoms%rmt,cell%amat,cell%bmat,cell%bbmat,nococonv%alph,&
            nococonv%beta,nococonv%qss,stars%sk2,stars%phi2,&
            fmpi%irank,&
    ...
    IF(wann%l_matrixmmn)THEN
       ...
       CALL wann_mmnk_symm(input,kpts,sym,&
            fullnkpts,nntot,bpt,gb,wann%l_bzsym,&
            irreduc,mapkoper,l_p0,input%film,&
 ...
 !         END IF
          ...
          nbasfcn = MERGE(lapw%nv(1)+lapw%nv(2)+2*atoms%nlotot,lapw%nv(1)+atoms%nlotot,noco%l_noco)
                ...
                CALL abcof(input,atoms,sym,cell,lapw,noccbd,usdus,&
                     noco,nococonv,jspin2 ,acof,bcof,ccof,zMat)
                ...
                IF(wann%l_surfcurr)THEN
                   ...
                   !     >        lapw%dim_nv2d(),jspin,stars%ng3,vacuum%nmzxyd,stars%ng2,sphhar%ntypsd,
...
!                      noconbasfcn=nbasfcn-addnoco
                   ...
                   CALL wann_mmk0_sph(&
                        atoms,noccbd,lmd,acof(1:noccbd,:,:),&
                   ...
                   IF (input%film ) THEN
                      ...
                      CALL wann_mmk0_vac(&
                           ...
                           cell,vacuum,lapw%dim_nv2d(),&
                           stars%mx1,stars%mx2,stars%mx3,&
                           stars%ng3,&
                           stars%ig,stars%ig2,&
                           enpara%evac0(:,jspin2),lapw%bkpt,vz(:,:,jspin2),&
                           nslibd,jspin2,lapw%k1,lapw%k2,lapw%k3,wannierspin,lapw%dim_nvd(),&
                           lapw%dim_nbasfcn(),input%neig,zMat,lapw%nv,&
                           mmn(:,:,ikpt))
                   ENDIF
                ENDIF !l_mmn0
                ...
                IF (wann%l_projmethod.OR.wann%l_bestproj.OR.wann%l_matrixamn) THEN
                   ...
                   CALL wann_amn (&
                        atoms,amnchi,nslibd,nwfs,lmd,ikpt,nbnd,&
...
!!! get the band-dependent k-dependent ab coeff.
                      ...
                      CALL abcof(input,atoms,sym,cell,lapw_b,&
                           noccbd_b,usdus,noco,nococonv,jspin2 ,&
...
!                         noconbasfcn_b=nbasfcn_b-addnoco2
                      ...
                      IF (input%film ) THEN
                         ...
                         CALL wann_mmkb_vac(&
                              ...
                              nbnd,cell,vacuum,lapw%dim_nv2d(),&
                              stars%mx1,stars%mx2,stars%mx3,&
                              stars%ng3,&
                              stars%ig,stars%ig2,enpara%evac0(:,jspin2),&
                ...
                IF (wann%l_matrixmmn) THEN   !  vanderbilt procedure Mmn matrix
                   ...
                   DO iqpt_b=1,nntot_q
                      ...
                      CALL abcof(input,atoms,sym,cell,lapw_qb,&
                           noccbd_qb,usdus,noco,nococonv,jspin_b ,&
                      ...
                      DO n=1,atoms%ntype
                         ...
                         ELSEIF( (jspin.EQ.2) .AND. (jspin_b.EQ.1) ) THEN ! du
                            ...
                            chi(n) = SIN(db2-db1)
                         ELSE
                            Call judft_error('problem setting up chi: jspin,jspin_b')
                         ENDIF
                      ENDDO
                      chi = CONJG(chi)
                      !chi = cmplx_1
                      ...
                      IF(wann%l_sgwf) CALL wann_mmkb_sph( &
                           atoms,nbnd,nslibd,nslibd_qb,lmd,&
                           sign_q*atoms%taual/2.0,sym%nop,&
                      ...
                      IF(((doublespin.NE.3).AND.(doublespin.NE.4))&
                         ...
                         IF (input%film ) THEN
                            IF(wann%l_sgwf) CALL wann_mmkb_vac(&
                                 ...
                                 stars%mx1,stars%mx2,stars%mx3,&
                                 stars%ng3,&
                                 stars%ig,stars%ig2,enpara%evac0(:,jspin),enpara%evac0(:,jspin_b),&
                                 sign_q*qpt_i/2.,&
                                 sign_q*qptb_i/2.,&
                   ...
                   IF (wann%l_wann_plot .AND. &
                      ...
                      IF (sliceplot%slice) THEN
                         IF (ikpt.EQ.sliceplot%kk) THEN
                            ...
                            CALL wann_plot(&
                                  vacuum,stars,cell,atoms,sym,&
                                 lapw%dim_nv2d(),jspin,sphhar%ntypsd,&
                                 input%film,sliceplot%slice,ikpt,sliceplot%nnne,&
                                 sliceplot%kk,lapw%dim_nvd(),&
                                 lapw%nv(jspin),lmd,lapw%bkpt,&
                                 lapw%k1(:,jspin),lapw%k2(:,jspin),lapw%k3(:,jspin),enpara%evac0(:,jspin),&
                                 vz(:,:,jspin2),&
                                 nslibd,lapw%dim_nbasfcn(),input%neig,&
                                 ff(:,:,:,:,jspin),&
                                 gg(:,:,:,:,jspin),flo(:,:,:,:,jspin),acof,bcof,ccof,zMat,&
...
1911   CONTINUE
       ...
       CALL wann_postproc(&
            stars,vacuum,atoms,sphhar,input,kpts,sym,fmpi,&
            lapw ,noco,nococonv,cell,vTot,enpara,sliceplot,eig_id,l_real,&                !eig_id is used here after closing the files?!&
