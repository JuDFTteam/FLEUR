...
!>  gauge and Fourier transformed to real space with the distributed reduce in m_wgauge_ft;
...
!>
!>  The b-shell weights and the reference Wannier centres arrive as a plain t_wgauge_bmesh,
!>  so this module is free of the Wannier90 library.
MODULE m_wgauge_operators_r
  USE m_juDFT
  ...
  USE m_wgauge_ft, ONLY : wgauge_ft_to_real, wgauge_ws_vectors, wgauge_ft_to_real_reduce
  USE m_wgauge_io, ONLY : wgauge_write_realspace
  USE m_wgauge_hamk, ONLY : wgauge_build_hamk
  USE m_wgauge_coeff_a, ONLY : wgauge_build_berry_aw_r, wgauge_check_berry_centres, wgauge_write_ar
  USE m_wgauge_coeff_b, ONLY : wgauge_write_bmn
...
CONTAINS
  ...
  SUBROUTINE wgauge_write_hr(this, cell, kpts, hr, irvec, ndegen, nrpts, wfpref)
    ...
    CALL wgauge_write_realspace(hr4, irvec, ndegen, nrpts, nw, 1, 'hr', TRIM(fn)//'.dat', 0)
    DEALLOCATE(hr4)
    WRITE(oUnit,'(a)') 'wannierlib: wrote '//TRIM(fn)//'.dat (H(R), eV)'
  END SUBROUTINE wgauge_write_hr
  ...
  ! ---------------------------------------------------------------------------
  SUBROUTINE wgauge_write_operators_r(this, request, cell, kpts, eig, u_matrix, u_opt, &
    ...
    TYPE(t_wgauge_bmesh), INTENT(IN) :: bmesh         ! b-shell weights (position/Berry operator)
    ...
    CALL timestart('wgauge_write_operators_r')
    nb = this%num_bands
    ...
    IF (irank == 0) THEN; l_wig_done = .FALSE.; CALL wgauge_write_wig_once(cell, kpts, l_wig_done); END IF
    ...
    !> rather than only where it is written: wgauge_write_bmn is collective, so all of them
    ...
    IF (l_need_hr) THEN
      ...
      CALL wgauge_ft_to_real(cell, ham_k, kpts, ham_r, hr_irvec, hr_ndegen, hr_nrpts)
      DEALLOCATE(ham_k)
    END IF
    DO iop = 1, request%n_op_r
      SELECT CASE (TRIM(request%op_r_name(iop)))
      CASE ('hamiltonian')   ! H(R) already built above
        IF (irank == 0) CALL wgauge_write_hr(this, cell, kpts, ham_r, hr_irvec, hr_ndegen, &
                                            hr_nrpts, TRIM(wfpref))
      CASE ('bmn')           ! collective: the k-sum is distributed like the others
        CALL wgauge_write_bmn(this, kpts, eig, u_matrix, u_opt, mmn_loc, gk_loc, &
      ...
      CASE ('fmn')           ! collective: only the sum over R is left, the gauge is already in
        CALL wgauge_write_fmn(this, kpts, f0_loc, gk_loc, hr_irvec, hr_nrpts, mpi_comm, &
                             irank, TRIM(wfpref))
      CASE ('cmn')           ! collective: like fmn, the gauge is already applied
        CALL wgauge_write_cmn(this, kpts, c0_loc, gk_loc, hr_irvec, hr_nrpts, mpi_comm, &
                             irank, TRIM(wfpref))
      CASE ('position')      ! Berry connection A^(W)(R)=A(R): distributed reduce over the local overlaps
        CALL wgauge_build_berry_aw_r(this, cell, kpts, mmn_loc, gk_loc, u_opt, u_matrix, bmesh, mpi_comm, &
        ...
        IF (irank == 0) CALL wgauge_check_berry_centres(this, aw_r, aw_irvec, aw_nrpts, bmesh)
        IF (irank == 0) CALL wgauge_write_ar(this, aw_r, aw_irvec, aw_nrpts, TRIM(wfpref))
      ...
      CASE ('position_pw90') ! A(R) in the postw90 convention: Eq. (44) on the diagonal too,
        ...
        IF (irank == 0) CALL wgauge_write_ar(this, aw_r, aw_irvec, aw_nrpts, TRIM(wfpref), '_rpw')
        IF (ALLOCATED(aw_r)) DEALLOCATE(aw_r, aw_irvec, aw_ndegen)
      CASE ('spin')          ! distributed reduce over the coarse spin slice
        ...
        ! after both wannierisations by wgauge_rspauli_collinear (main), not per channel here.
        IF (l_collinear) THEN
          CONTINUE
        ELSE
          CALL wgauge_op_rs_distributed(this, cell, kpts, vloc, s0_loc, gk_loc, 3, mpi_comm, irank, .FALSE., 'rspauli.1')
        END IF
      CASE ('orbital')
        ! collinear: like the spin, L is written once as one 2N block-diagonal matrix after
        ! both wannierisations by wgauge_anglmom_collinear (main), not per channel here.
        ...
        CALL wgauge_op_rs_distributed(this, cell, kpts, vloc, o0l, gk_loc, 3, mpi_comm, irank, .FALSE., &
                                          'anglmomrs.'//ACHAR(48+wf_channel))
        DEALLOCATE(o0l)
      CASE ('spin_orbit')
        ! collinear: like spin and orbital, written once as one 2N matrix after both
        ! wannierisations by wgauge_soc_collinear (main), not per channel here.
        IF (l_collinear) THEN
          CONTINUE
        ELSE
          CALL wgauge_op_rs_distributed(this, cell, kpts, vloc, soc4_loc, gk_loc, 4, mpi_comm, irank, .TRUE., 'rssocmat.1')
        END IF
      CASE DEFAULT
        !> Accepted by the request, so the gap is this dispatch and not the input.
        CALL judft_bug('wgauge_operators_r: "'//TRIM(request%op_r_name(iop))// &
    ...
    CALL timestop('wgauge_write_operators_r')
  END SUBROUTINE wgauge_write_operators_r
  ...
  SUBROUTINE wgauge_op_rs_distributed(this, cell, kpts, vloc, o0_loc, gk_loc, ncomp, mpi_comm, irank, is_soc, fname)
    TYPE(t_wgauge_manifold), INTENT(IN) :: this
    TYPE(t_cell), INTENT(IN) :: cell
    TYPE(t_kpts), INTENT(IN) :: kpts
    ...
    DEALLOCATE(tmp)
    DO a = 1, ncomp
      CALL wgauge_ft_to_real_reduce(cell, kpts, ow_loc(:, :, a, :), gk_loc, mpi_comm, o1, irvec, ndegen, nrpts)
      IF (a == 1) ALLOCATE(or_(nw, nw, nrpts, ncomp))
      or_(:, :, :, a) = o1; DEALLOCATE(o1)
    END DO
    DEALLOCATE(ow_loc)
    IF (irank == 0) THEN
      CALL wgauge_write_realspace(or_, irvec, ndegen, nrpts, nw, ncomp, &
  ...
  END SUBROUTINE wgauge_op_rs_distributed

  ! wig_vectors (once): idx R1 R2 R3 ndegen (list-directed; = orbitrans rpts).
  SUBROUTINE wgauge_write_wig_once(cell, kpts, done)
    TYPE(t_cell), INTENT(IN) :: cell
    TYPE(t_kpts), INTENT(IN) :: kpts
    ...
    INTEGER :: nrpts, ii, iu
    IF (done) RETURN
    CALL wgauge_ws_vectors(cell, kpts%nkpt3, irvec, ndegen, nrpts)
  ...
  END SUBROUTINE wgauge_write_wig_once

END MODULE m_wgauge_operators_r
