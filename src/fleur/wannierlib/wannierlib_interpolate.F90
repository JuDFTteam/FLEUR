...
!>  Fourier core m_wannierlib_ft. It reproduces Wannier90's get_hr to build the
...
!>  wannierlib_ft_interpolate, diagonalizes H(k') and writes the bands.
!>
!>  To interpolate a different operator, write an analogous driver that builds
!>  its own operator matrix O_W(k) and reuses wannierlib_ft_interpolate unchanged.
!>
!>  Runs only on the master rank (irank==0), where the W90 U matrices are complete.
MODULE m_wannierlib_interpolate
  USE m_juDFT
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: wannierlib_interpolate
CONTAINS

  SUBROUTINE wannierlib_interpolate(this, cell, kpts, eig, u_matrix, u_opt, irank)
    ...
    CALL timestart('wannierlib_interpolate')
    ...
    IF (.NOT. lexist) THEN
      ...
      CALL timestop('wannierlib_interpolate'); RETURN
    END IF
    OPEN(newunit=iu, file='kpts_interpol', status='old')
    READ(iu, *, iostat=ios) np
    IF (ios /= 0 .OR. np <= 0) CALL juDFT_error('bad kpts_interpol header', calledby='wannierlib_interpolate')
    ALLOCATE(kfrac(3, np))
    DO ip = 1, np
      READ(iu, *, iostat=ios) kfrac(:, ip)
      IF (ios /= 0) CALL juDFT_error('bad kpts_interpol line', calledby='wannierlib_interpolate')
    END DO
    CLOSE(iu)
    ...
    DO ip = 1, np
      ...
      IF (info /= 0) CALL juDFT_error('zheev failed', calledby='wannierlib_interpolate')
      IF (ip > 1) THEN
    ...
    CALL timestop('wannierlib_interpolate')
  END SUBROUTINE wannierlib_interpolate

END MODULE m_wannierlib_interpolate
