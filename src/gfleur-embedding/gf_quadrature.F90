...
!                                                                       
      lvl = 0 
   10 IF(ier==6) lvl = 1 
      IF(ier/=0)  CALL juDFT_error("dqage",calledby="quadrature.F90")
      RETURN 
      END SUBROUTINE 
...
!                                                                       
!***references  (none)                                                  
!***routines called  d1mach_private,dqk15,dqk21,dqk31,                  
...
!***first executable statement  dqage                                   
      epmach = d1mach_private(4) 
      uflow = d1mach_private(1) 
!                                                                       
...
!                                                                       
!***references  (none)                                                  
!***routines called  d1mach_private                                     
!***end prologue  dqk15                                                 
!                                                                       
      REAL a,absc,abserr,b,centr,dabs,dhlgth,dmax1,dmin1,               &
...
!                                                                       
   ...
   20 CONTINUE 
     ...
     &  ((epmach*0.5d+02)*resabs,abserr*1.0d+00)                        
      RETURN 
      END SUBROUTINE 
...
!***end prologue  dqk61                                                 
!                                                                       
      REAL a,dabsc,abserr,b,centr,dabs,dhlgth,dmax1,dmin1,              &
     &  epmach,f,fc,fsum,fval1,fval2,fv1,fv2,hlgth,resabs,resasc,       &
     &  resg,resk,reskh,result,uflow,wg,wgk,xgk                         
      INTEGER j,jtw,jtwm1 
      EXTERNAL f 
...
!DECK D1MACH_PRIVATE                                                    
      REAL FUNCTION D1MACH_PRIVATE (I) 
      IMPLICIT NONE 
      INTEGER :: I 
      REAL :: B, X 
!***BEGIN PROLOGUE  D1MACH_PRIVATE                                      
!***PURPOSE  Return floating point machine dependent constants.         
!***LIBRARY   SLATEC                                                    
!***CATEGORY  R1                                                        
!***TYPE      SINGLE PRECISION (D1MACH_PRIVATE-S, D1MACH_PRIVATE-D)     
...
!***DESCRIPTION                                                         
!                                                                       
!   D1MACH_PRIVATE can be used to obtain machine-dependent parameters fo
...
!   discussed below.                                                    
!                                                                       
!   D1MACH_PRIVATE(1) = B**(EMIN-1), the smallest positive magnitude.   
!   D1MACH_PRIVATE(2) = B**EMAX*(1 - B**(-T)), the largest magnitude.   
!   D1MACH_PRIVATE(3) = B**(-T), the smallest relative spacing.         
!   D1MACH_PRIVATE(4) = B**(1-T), the largest relative spacing.         
!   D1MACH_PRIVATE(5) = LOG10(B)                                        
!                                                                       
...
!                                                                       
      ...
      SELECT CASE (I) 
        CASE (1) 
          ...
          D1MACH_PRIVATE = B**(MINEXPONENT(X)-1) 
        CASE (2) 
                                                 ! the largest magnitude
          D1MACH_PRIVATE = HUGE(X) 
        CASE (3) 
                                                 ! the smallest relative
          D1MACH_PRIVATE = B**(-DIGITS(X)) 
        CASE (4) 
                                                 ! the largest relative 
          D1MACH_PRIVATE = B**(1-DIGITS(X)) 
        CASE (5) 
