...
# One-liner: creates and displays the full tabbed GUI
gui = fleuriste.jupyter(schema="FleurInputSchema.xsd", input_file="inp.xml")

# Or use individual panels
from fleuriste.jupyter_gui import KPointPanel
kpt = KPointPanel("inp.xml")
kpt.widget
```

A ready-made demo notebook is available at `notebooks/fleuriste_jupyter.ipynb`.
...
├── jupyter_gui.py    # JupyterLab ipywidgets interface
...
- `textual-plotext` - For k-point visualization plots (TUI)
- `ipywidgets` + `matplotlib` - For JupyterLab interface
