...
CONTAINS
  ...
  SUBROUTINE make_defaults(atoms, sym, cell, vacuum, input, stars, xcpot, profile, noco, banddos, mpinp, hybinp,sliceplot)
    USE m_types_atoms
    USE m_types_cell
    USE m_types_sym
    USE m_types_vacuum
    ...
    !Preconditioning parameter for films
    IF (input%film) input%preconditioning_param = 0.8
    !
    !noco
    !
