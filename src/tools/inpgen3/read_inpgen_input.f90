...
CONTAINS
  ...
  SUBROUTINE read_inpgen_input(profile,atom_pos,atom_id,mag_mom,atom_label,kpts_str,kptsName,kptsPath,kptsBZintegration,&
                               kptsGamma,input,sym,noco,vacuum,stars,xcpot,cell,hybinp,file_id)
    ...
    CALL  normalize_file(file_id,98)
    CLOSE(file_id)
    ...
    readloop: DO WHILE(ios==0)
       ...
       IF (line(1:1)=="&") THEN
          ...
          CASE('kpt ')
             iKpts = iKpts + 1
             if (iKpts>1 ) call judft_error("Only one k-point sets supported in inpgen")
             CALL process_kpts(line,kpts_str,kptsName,kptsPath,kptsBZintegration,kptsGamma,input%tkb)
             IF(TRIM(ADJUSTL(kptsName)).EQ.'') THEN
                IF(TRIM(ADJUSTL(kptsPath)).EQ.'') THEN
                   kptsName="default-1"
                ELSE
                   kptsName="path-1"
                END IF
       ...
       ENDIF
    END DO readloop
    !Generate default k-point set if kpoints where not specified
