...
- Lattice: cubic parent a = 4.334 Å

## Derivation of the type-II AFM magnetic cell

FeO is rock salt (Fm-3m, #225) with the Fe cation on the
...
units of the cubic lattice constant a = 4.334 Angstrom):
...
```
Fe (spin up)    (0,    0,    0   )
Fe (spin down)  (0.5,  0.5,  0.5 )
O                       (0.25, 0.25, 0.25)
O                       (0.75, 0.75, 0.75)
```

This cell has volume 0.5 a^3 (2 formula units, as required for two
magnetically inequivalent Fe sites) and a rhombohedral angle
...
- **SCAN (Tran et al. 2020, Table VI), Bader spin moment on Fe: 3.62 muB**
- Experiment (total mu_S + mu_L): 3.32 / 4.2 / 4.6 (footnotes b-d)
- **Comparison quantity in FLEUR: use the muffin-tin projected spin
  ...
  `out.xml`. Expect a systematic offset from the 3.62 muB target
