...
- Lattice: cubic parent a = 4.445 Å

## Derivation of the type-II AFM magnetic cell

MnO is rock salt (Fm-3m, #225) with the Mn cation on the
...
units of the cubic lattice constant a = 4.445 Angstrom):
...
```
Mn (spin up)    (0,    0,    0   )
Mn (spin down)  (0.5,  0.5,  0.5 )
O                       (0.25, 0.25, 0.25)
O                       (0.75, 0.75, 0.75)
```

This cell has volume 0.5 a^3 (2 formula units, as required for two
magnetically inequivalent Mn sites) and a rhombohedral angle
...
- **SCAN (Tran et al. 2020, Table VI), Bader spin moment on Mn: 4.53 muB**
- Experiment (total mu_S + mu_L): 4.58 (footnote a)
- **Comparison quantity in FLEUR: use the muffin-tin projected spin
  magnetic moment on the Mn atom** (per your instruction), not a
  Bader-volume moment -- FLEUR reports this directly per atom type in
  `out.xml`. Expect a systematic offset from the 4.53 muB target
