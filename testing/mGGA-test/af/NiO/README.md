...
- Lattice: cubic parent a = 4.171 Å

## Derivation of the type-II AFM magnetic cell

NiO is rock salt (Fm-3m, #225) with the Ni cation on the
...
units of the cubic lattice constant a = 4.171 Angstrom):
...
```
Ni (spin up)    (0,    0,    0   )
Ni (spin down)  (0.5,  0.5,  0.5 )
O                       (0.25, 0.25, 0.25)
O                       (0.75, 0.75, 0.75)
```

This cell has volume 0.5 a^3 (2 formula units, as required for two
magnetically inequivalent Ni sites) and a rhombohedral angle
...
- **SCAN (Tran et al. 2020, Table VI), Bader spin moment on Ni: 1.6 muB**
- Experiment (total mu_S + mu_L): 1.9 / 2.2 (footnotes h,i)
- **Comparison quantity in FLEUR: use the muffin-tin projected spin
  magnetic moment on the Ni atom** (per your instruction), not a
  Bader-volume moment -- FLEUR reports this directly per atom type in
  `out.xml`. Expect a systematic offset from the 1.6 muB target
