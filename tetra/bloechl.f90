MODULE m_bloechl
   ...
   PURE REAL FUNCTION bloechl(efermi,etetra,ind,vol,film)
      ...
      bloechl = 0.0
      DO i = 1, 4
         bloechl = bloechl + 1/40.0*dos_tetra(efermi,etetra,vol)*(etetra(i)-etetra(ind))
      ENDDO
   ...
   PURE REAL FUNCTION dos_tetra(energy,etetra,vol)
      ...
      IF((energy.GT.etetra(4)).OR.(energy.LT.etetra(1))) THEN
         dos_tetra = 0.0
      ELSE IF((energy.GT.etetra(1)).AND.(energy.LT.etetra(2))) THEN

         dos_tetra = 3.0 * vol * (energy-etetra(1))**2/((etetra(2)-etetra(1))*(etetra(3)-etetra(1))*(etetra(4)-etetra(1)))

      ELSE IF((energy.GT.etetra(2)).AND.(energy.LT.etetra(3))) THEN
      ...
      ELSE IF((energy.GT.etetra(3)).AND.(energy.LT.etetra(4))) THEN
