...
CONTAINS
   ...
   SUBROUTINE mpimat_multiply(mat1, mat2, res, transA, transB)
      ...
      TYPE IS (t_mpimat)
         if(transB_i == "N" ) then
            if(k /= mat2%global_size1) call judft_error("dimensions don't agree for matmul")
               n = mat2%global_size2
         else
            if(k /= mat2%global_size2) call judft_error("dimensions don't agree for matmul")
            n = mat2%global_size1
         endif
         SELECT TYPE (res)
         TYPE is (t_mpimat)
            IF (mat1%l_real) THEN
               CALL pdgemm(transA_i, transB_i, m, n, k, 1.0, &
                           mat1%data_r, 1, 1, mat1%blacsdata%blacs_desc, &
                           mat2%data_r, 1, 1, mat2%blacsdata%blacs_desc, 0.0, &
                           res%data_r, 1, 1, res%blacsdata%blacs_desc)
            ELSE
               CALL pzgemm(transA_i, transB_i, m, n, k, cmplx_1, &
                           mat1%data_c, 1, 1, mat1%blacsdata%blacs_desc, &
                           mat2%data_c, 1, 1, mat2%blacsdata%blacs_desc, cmplx_0, &
                           res%data_c, 1, 1, res%blacsdata%blacs_desc)
            END IF
