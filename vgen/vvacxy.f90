...
contains
   subroutine vvacxy( stars, vacuum, cell, sym, input, field, rhtxy, vxy, alphm, l_dfptvgen )
      ...
      complex,        intent(in)     :: rhtxy(vacuum%nmzxyd,stars%ng2,2)
      ...
      if ( field%efield%dirichlet ) then ! Dirichlet
         do irec2 = start_star, stars%ng2
            ...
            do ivac = 1, vacuum%nvac
               ...
               do imz = 1, ncsh-1
                  ...
                  fib(imz)      = aimag( rhtxy(imz,irec2,ivac) ) * e_p
                  z = z + vacuum%delz
               end do 
         ...
         do irec2 = start_star, stars%ng2
            ...
            do ivac = 1, vacuum%nvac
               z = cell%z1
               if ( ivac == 1 ) alph0 = alphm(irec2,2)
               if ( ivac == 2 ) alph0 = alphm(irec2,1)
               do imz = 1, ncsh-1
                  ...
                  if ( allocated( field%efield%c1 ) ) then
                     ...
                     e_p = exp_safe(   g * z )
                     if ( ivac == 1 ) then ! z > 0
                        vxy(imz,irec2,ivac) = vxy(imz,irec2,ivac) + field%efield%c1(irec2-1) * e_p + field%efield%c2(irec2-1) * e_m
                     else ! z < 0
                        vxy(imz,irec2,ivac) = vxy(imz,irec2,ivac) + field%efield%c1(irec2-1) * e_m + field%efield%c2(irec2-1) * e_p
                     end if
                  end if
                  z = z + vacuum%delz
               end do 
         ...
         do irec2 = 2, stars%ng2
            ...
            do ivac = 1, vacuum%nvac
               ...
               do imz = 1, vacuum%nmzxy
                  e_m = exp_safe( - g * z )
                  e_p = exp_safe(   g * z )
                  fra(ip-imz) = real(  rhtxy(imz,irec2,ivac) ) * e_m
                  fia(ip-imz) = aimag( rhtxy(imz,irec2,ivac) ) * e_m
                  frb(imz)    = real(  rhtxy(imz,irec2,ivac) ) * e_p
                  fib(imz)    = aimag( rhtxy(imz,irec2,ivac) ) * e_p
                  z = z + vacuum%delz
               end do 
