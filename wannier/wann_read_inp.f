...
!--------------------------------------------------------------------------------
     ...
     <               l_p0,wann)
c********************************************
c     Read the Wannier input file 'wann_inp'.
c     Frank Freimuth
c********************************************
      use m_types
...
c-----read the input file 'wann_inp'
      ...
      if (.not. l_file) CALL juDFT_error ("wann_inp not found",
     +                                  calledby="wann_read_inp")
      ...
      do 
         ...
         if(index(task,'=F').ne.0
     &    .or.index(task,'=f').ne.0)cycle
         ...
         elseif(trim(task).eq.'kptsreduc2')then
            ...
            backspace(916)
            read(916,*,iostat=ios)task,wann%mhp(1),wann%mhp(2),
     &                                 wann%mhp(3)
            if (ios /= 0) CALL juDFT_error ("error reading mhp",
     &                                    calledby="wann_read_inp")
            if(l_p0)write(6,*)"mhp=",wann%mhp(1),wann%mhp(2),
     &                               wann%mhp(3)
         ...
         elseif(trim(task).eq.'lapw_gfleur')then
            ...
            if (ios /= 0) CALL juDFT_error ("error reading gfcut",
     &                                    calledby="wann_read_inp")
         ...
         elseif(trim(task).eq.'lapw')then
            ...
            if (ios /= 0) CALL juDFT_error ("error reading unigrid",
     &                                    calledby="wann_read_inp")
     ...
     &                        calledby="wann_read_inp")
         ...
         elseif(trim(task).eq.'byindex2')then
            ...
            if (ios /= 0)
     &        CALL juDFT_error ("error reading byindex2,band_min2,"
     &                        //"band_max", calledby="wann_read_inp")
         ...
         elseif(trim(task).eq.'ikptstart')then
            ...
            if (ios /= 0)
     &        CALL juDFT_error ("error reading ikptstart",
     &                        calledby="wann_read_inp")
            if(l_p0)write(6,*)"ikptstart=",wann%ikptstart
         else
            write(6,*)"unrecognized key: ",task
            CALL juDFT_error ("unrecognized key in wann_inp",
     &                        calledby="wann_read_inp")
         endif
      enddo

      if (ios /= 0) CALL juDFT_error ("error reading wann_inp",
     &                              calledby="wann_read_inp")
