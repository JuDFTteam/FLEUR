...
CONTAINS
   ...
   SUBROUTINE pw_kinED(stars,kpts,banddos,oneD, input,mpi,noco,cell,atoms,sym, &
...
#include"cpp_double.h"
    ...
    IF (noco%l_noco) THEN
       call judft_error("no noco implementation 1")
    ELSE
       IF (zmat%l_real) THEN
    ...
    ndens = 1
    IF (noco%l_noco) call judft_error("no noco 4")
    DO idens = 1,ndens
       IF (noco%l_noco) THEN
          call judft_error("no noco 5")
       ELSE
       ...
       !
       IF ((idens == 1).OR.(idens == 2)) THEN
          IF (noco%l_noco) THEN
             call judft_error("no noco 7")
          ENDIF
    ...
    IF (noco%l_noco) THEN
       call judft_error("no nocoi last")
    ELSE
