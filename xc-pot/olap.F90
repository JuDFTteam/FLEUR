        ...
        DO j=1,i
          ...
          IF(g.eq.0) THEN
            DO itype=1,atoms%ntype
              r          = atoms%rmt(itype)
              olap(i,j) = olap(i,j) - atoms%neq(itype) *fpi_const*r**3/3/cell%omtil
            END DO
          ELSE
            icent = 0
            DO itype=1,atoms%ntype
...
!     olap_pwp  calculates upper triangular part of overlap matrix
      ...
      SUBROUTINE olap_pwp(olap,gpt,ngpt,atoms,cell)
      ...
      DO i=1,ngpt
        DO j=1,i
          ...
          IF(g.eq.0) THEN
            DO itype=1,atoms%ntype
              r       = atoms%rmt(itype)
              olap(k) = olap(k) - atoms%neq(itype) * fpi_const*r**3/3 / cell%omtil
            END DO
          ELSE
            icent = 0
            DO itype=1,atoms%ntype
...
#if ( defined(CPP_INVERSION) )
      REAL,INTENT(OUT)         :: olappw(size(gpt,2),size(gpt,2))
#else
      COMPLEX,INTENT(OUT)      :: olappw(size(gpt,2),size(gpt,2))
#endif
...
!               olapmt(n2,n1,l,itype) = olapmt(n1,n2,l,itype)
      ...
      FUNCTION wfolap(cmt1,cpw1,cmt2,cpw2,ngpt1,ngpt2,olappw,olapmt,atoms,hybrid)
